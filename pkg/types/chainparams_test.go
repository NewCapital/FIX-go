...
func TestChainParamsGetTierCollateral(t *testing.T) {
	...
	if bronzeCollateral != 100000000000000 { // 1M FIX in satoshis
	...
	if silverCollateral != 500000000000000 { // 5M FIX in satoshis
	...
	if goldCollateral != 2000000000000000 { // 20M FIX in satoshis
	...
	if platinumCollateral != 10000000000000000 { // 100M FIX in satoshis
...
func TestMainnetParams(t *testing.T) {
	...
	if params.DefaultPort != 17464 {
		t.Errorf("Expected default port 17464, got %d", params.DefaultPort)
	}
	...
	const expectedGenesis = "000000428366d3a156c38c5061d74317d201781f539460aeeeaae1091de6e4cc"
	...
	// FIX legacy chainparams.cpp:227 — nNonce
	if params.GenesisNonce != 3617423 {
		t.Errorf("expected genesis nonce 3617423, got %d", params.GenesisNonce)
	}

	// FIX legacy chainparams.cpp:226 — nTime
	if params.GenesisTimestamp != 1559224740 {
		t.Errorf("expected genesis timestamp 1559224740, got %d", params.GenesisTimestamp)
	}

	// Fork divergence: legacy chainparams.cpp:186 sets nStakeMinInput = 100 * COIN,
	// but fix-go uses 12000 * COIN per commit 352a16ce "legacy v..10 revert".
	if params.MinStakeAmount != 12000*100000000 {
		t.Errorf("expected MinStakeAmount = 12000 * COIN, got %d", params.MinStakeAmount)
	}

	// FIX legacy chainparams.cpp:157-160 — pchMessageStart
	expectedMagic := [4]byte{0x74, 0x2c, 0x4d, 0x64}
	if params.NetMagicBytes != expectedMagic {
		t.Errorf("expected magic bytes %v, got %v", expectedMagic, params.NetMagicBytes)
	}
}
...
func TestTestnetParams(t *testing.T) {
	...
	if params.DefaultPort != 5447 {
		t.Errorf("Expected default port 5447, got %d", params.DefaultPort)
	}
	...
	// FIX legacy chainparams.cpp:308-311 — testnet pchMessageStart
	expectedTestnetMagic := [4]byte{0x44, 0x6a, 0xa4, 0xcc}
	if params.NetMagicBytes != expectedTestnetMagic {
		t.Errorf("expected testnet magic %v, got %v", expectedTestnetMagic, params.NetMagicBytes)
	}

	// FIX legacy chainparams.cpp:355 — testnet hashGenesisBlock
	const expectedTestnetGenesis = "000002849e7ad33536de6c50b3efb55fe8f20f219de408be70a6614c105e6bff"
	if params.GenesisHash.String() != expectedTestnetGenesis {
		t.Errorf("expected testnet genesis hash %s, got %s", expectedTestnetGenesis, params.GenesisHash.String())
	}
...
func TestRegtestParams(t *testing.T) {
	...
	// FIX legacy chainparams.cpp:421-424 — regtest pchMessageStart
	expectedRegtestMagic := [4]byte{0x2a, 0xfc, 0xc7, 0xca}
	if params.NetMagicBytes != expectedRegtestMagic {
		t.Errorf("expected regtest magic %v, got %v", expectedRegtestMagic, params.NetMagicBytes)
	}

	// Test minimal collateral requirements
	bronzeCollateral := params.GetTierCollateral(MasternodeTierBronze)
	if bronzeCollateral != 100000000 { // 1 FIX in satoshis
		t.Errorf("Expected regtest Bronze collateral 100000000, got %d", bronzeCollateral)
	}

	platinumCollateral := params.GetTierCollateral(MasternodeTierPlatinum)
	if platinumCollateral != 10000000000 { // 100 FIX in satoshis
