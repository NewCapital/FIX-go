...
func fixedHash() []byte {
	h := sha256.Sum256([]byte("fix-go tx signing round-trip digest"))
	return h[:]
}

// TestWalletSignPath_ProducesVerifiableSignature is the critical reproduction.
// It runs the exact path used by Wallet.SignTransaction (PrivateKey.Sign + Signature.Bytes)
// and verifies the resulting DER bytes against btcec's strict ParseDERSignature and Verify.
// Legacy C++ uses ecdsa_signature_parse_der_lax + ecdsa_signature_normalize + secp256k1_ecdsa_verify,
// which is STRICTLY MORE PERMISSIVE than btcec's strict path. If this test passes, legacy should
// accept the signature. If it fails, we have found the bug.
func TestWalletSignPath_ProducesVerifiableSignature(t *testing.T) {
	...
	// Strict DER parse via btcec (same library/algorithm legacy uses).
	...
	// Verify against the original pubkey + hash.
...
// TestSignCompact_vs_Sign_ProduceSameDER establishes whether the two signing paths
// (ecdsa.SignCompact used by PrivateKey.Sign, vs ecdsa.Sign used by FIXMessageSigner.Sign)
// produce byte-identical DER signatures for the same key and hash.
// If they diverge, we have located the class of bug.
func TestSignCompact_vs_Sign_ProduceSameDER(t *testing.T) {
	priv := fixedKey(t)
	hash := fixedHash()

	// Path A: canonical ecdsa.Sign + sig.Serialize (used by FIXMessageSigner.Sign)
...
// If this fails, the wallet is producing high-S signatures which legacy would still
// normalize before verifying — but it would point us at a different suspect area.
...
// TestManualDER_Equivalent_To_NativeDER checks whether the manual DER encoder in
// Signature.Bytes() produces identical output to btcec's native sig.Serialize()
// given the same R and S values. This isolates DER-encoding edge cases from signing.
func TestManualDER_Equivalent_To_NativeDER(t *testing.T) {
	priv := fixedKey(t)
	hash := fixedHash()

	// Sign canonically and extract native DER.
	native := ecdsa.Sign(priv, hash)
	nativeDER := native.Serialize()

	// Parse native DER back out so we have R, S in our Signature struct, then re-encode
	// via our manual encoder. Both should yield the same byte string.
