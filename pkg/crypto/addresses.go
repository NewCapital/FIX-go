...
// Address network ID constants (FIX legacy values from chainparams.cpp:247-253, 375-383)
const (
	// FIX MainNet address prefixes — addresses begin with "F"
	MainNetPubKeyHashAddrID = 0x23 // 35 decimal - F... addresses
	MainNetScriptHashAddrID = 0x5f // 95 decimal

	// FIX TestNet address prefixes
	TestNetPubKeyHashAddrID = 0x4c // 76 decimal
	TestNetScriptHashAddrID = 0x89 // 137 decimal

	// Private key WIF prefix (FIX mainnet SECRET_KEY)
	PrivateKeyID = 0x3c // 60 decimal
)

// BIP32 HD wallet prefixes (FIX legacy from chainparams.cpp:251-252)
var (
	...
	// BIP44 coin type for FIX (from chainparams.cpp:254)
	CoinType = uint32(336)
)
...
// Address represents a FIX address
type Address struct {
	hash    []byte
	netID   byte
	version byte
