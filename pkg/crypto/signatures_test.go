...
func TestFIXMessageSigner(t *testing.T) {
	keyPair, err := GenerateKeyPair()
	if err != nil {
		t.Fatalf("GenerateKeyPair failed: %v", err)
	}

	signer := NewFIXMessageSigner(keyPair.Private)
	...
	// Test SignMessage
	message := []byte("test message for FIX signer")
...
func BenchmarkFIXMessageSigner(b *testing.B) {
	keyPair, err := GenerateKeyPair()
	if err != nil {
	...
	signer := NewFIXMessageSigner(keyPair.Private)
	message := []byte("benchmark message for FIX signer")
	opts := NewSignerOpts(crypto.SHA256)
