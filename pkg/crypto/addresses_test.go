...
func TestAddressValidation(t *testing.T) {
	...
	fixAddr := NewAddressFromPubKey(keyPair.Public, MainNetPubKeyHashAddrID)

	mainnetStr := mainnetAddr.String()
	testnetStr := testnetAddr.String()
	fixStr := fixAddr.String()
	...
	if !IsValidMainNetAddress(fixStr) {
		t.Error("Should validate FIX mainnet address")
	}
