...
// FIXMessageSigner implements Go 1.25's crypto.MessageSigner interface
type FIXMessageSigner struct {
...
// SignerOpts implements crypto.SignerOpts for FIX-specific options
type SignerOpts struct {
	hash         crypto.Hash
	masternodeID string
}

// NewFIXMessageSigner creates a new message signer from a private key
func NewFIXMessageSigner(privateKey *PrivateKey) *FIXMessageSigner {
	return &FIXMessageSigner{
		privateKey: privateKey.key,
	}
}

// Public returns the public key corresponding to the private key
func (s *FIXMessageSigner) Public() crypto.PublicKey {
	s.mutex.RLock()
...
func (s *FIXMessageSigner) Sign(reader io.Reader, digest []byte, opts crypto.SignerOpts) ([]byte, error) {
	s.mutex.RLock()
	defer s.mutex.RUnlock()
...
func (s *FIXMessageSigner) SignMessage(message []byte, opts crypto.SignerOpts) ([]byte, error) {
	s.mutex.RLock()
	defer s.mutex.RUnlock()
...
// GetPublicKey returns the public key as a FIX PublicKey type
func (s *FIXMessageSigner) GetPublicKey() *PublicKey {
	s.mutex.RLock()
	defer s.mutex.RUnlock()
...
func NewMasternodeSigner(privateKey *PrivateKey, nodeID string) *MasternodeSigner {
	return &MasternodeSigner{
		signer: NewFIXMessageSigner(privateKey),
		nodeID: nodeID,
	}
}
...
func SignMessage(privKey *btcec.PrivateKey, message []byte) ([]byte, error) {
	...
	// FIXMessageSigner.Sign() now returns 65-byte compact signature with proper recovery byte
	return sig, nil
}
