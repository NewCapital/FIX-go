...
func TestCLIApplicationsIntegration(t *testing.T) {
	...
	binDir := t.TempDir()
	fixdPath, fixCliPath := buildCLIApplications(t, binDir)

	t.Run("fixd", func(t *testing.T) {
		testFixdCLI(t, fixdPath)
	})

	t.Run("fix-cli", func(t *testing.T) {
		testFixCLI(t, fixCliPath)
	})
}
...
func buildCLIApplications(t *testing.T, binDir string) (fixdPath, fixCliPath string) {
	t.Helper()
	apps := map[string]string{
		"./cmd/fixd":    filepath.Join(binDir, "fixd"),
		"./cmd/fix-cli": filepath.Join(binDir, "fix-cli"),
	}
	...
	return apps["./cmd/fixd"], apps["./cmd/fix-cli"]
}

func testFixdCLI(t *testing.T, binaryPath string) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:     []string{"version"},
			wantExit: 0,
			contains: []string{"FIX Core"},
		},
		{
			name:     "help command",
			args:     []string{"--help"},
			wantExit: 0,
			contains: []string{"FIX cryptocurrency daemon", "COMMANDS:", "start", "stop"},
		},
		{
			name:     "status command",
			args:     []string{"status"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cmd.Env = []string{
				"FIX_LOG_LEVEL=error", // Reduce log noise in tests
			}
...
func testFixCLI(t *testing.T, binaryPath string) {
	tests := []struct {
		name     string
		args     []string
		wantExit int
		...
		},
		{
			...
			contains: []string{"FIX cryptocurrency RPC client", "COMMANDS:", "getinfo"},
		},
		{
...
func TestCLIEnvironmentVariables(t *testing.T) {
	...
	tmpDir := t.TempDir()
	binaryPath := filepath.Join(tmpDir, "fix-cli")

	// Build fix-cli
	buildCmd := newGoBuildCommand(t, "", "build", "-o", binaryPath, "../cmd/fix-cli")
...
`
	...
	binaryPath := filepath.Join(tmpDir, "fixd")

	// Build fixd
	buildCmd := newGoBuildCommand(t, "", "build", "-o", binaryPath, "../cmd/fixd")
	require.NoError(t, buildCmd.Run())
	...
	output := stdout.String() + stderr.String()
	assert.Contains(t, output, "FIX Core")
}

// Benchmark CLI startup performance
func BenchmarkFixdStartup(b *testing.B) {
	tmpDir := b.TempDir()
	binaryPath := filepath.Join(tmpDir, "fixd")

	// Build fixd
	buildCmd := newGoBuildCommand(b, "", "build", "-o", binaryPath, "../cmd/fixd")
	...
	for i := 0; i < b.N; i++ {
		...
		cmd.Env = []string{"FIX_LOG_LEVEL=error"}
