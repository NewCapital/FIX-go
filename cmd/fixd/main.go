...
func main() {
	app := clilib.CreateBaseApp("fixd", "FIX cryptocurrency daemon", clilib.String())

	// Prepare daemon-specific flags
	daemonFlags := []cli.Flag{}
	daemonFlags = append(daemonFlags, clilib.CommonDaemonFlags()...)
	daemonFlags = append(daemonFlags, clilib.CommonWalletFlags()...)
	daemonFlags = append(daemonFlags, clilib.DatabaseFlags()...)
	daemonFlags = append(daemonFlags, clilib.PerformanceFlags()...)

	app.Commands = []*cli.Command{
		{
			Name:  "start",
			Usage: "Start the FIX daemon",
			Flags: daemonFlags,
			Before: func(c *cli.Context) error {
				return clilib.SetupLogging(c)
			},
	...
	// of position (e.g., "fixd start --config=/path" works like
	// "fixd --config=/path start").
...
// CLIOnlyConfig holds transient settings not persisted in fixd.yml.
...
// buildConfigManager creates a ConfigManager loaded from fixd.yml and applies CLI overrides.
...
func buildConfigManager(c *cli.Context, dataDir string) (*config.ConfigManager, *CLIOnlyConfig, error) {
	// Determine YAML path
	var yamlPath string
	explicitConfig := clilib.ConfigWasExplicitlySet(c)
	if explicitConfig {
		yamlPath = clilib.GetConfigPath(c)
	} else {
		yamlPath = filepath.Join(dataDir, "fixd.yml")
	}
	...
	if c.IsSet("reservebalance") {
		...
		if rb > types.MaxReserveBalanceSatoshis {
			return nil, nil, fmt.Errorf("invalid value for --reservebalance: cannot exceed 100M FIX (%d satoshis), got %d", types.MaxReserveBalanceSatoshis, rb)
		}
...
func stopDaemon(c *cli.Context) error {
	...
	fmt.Println("✓ FIX daemon stopped")
	return nil
}
...
func getDaemonStatus(c *cli.Context) error {
	...
	fmt.Println("✓ FIX daemon is running")
