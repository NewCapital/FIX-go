...
// from settings.json (GUI preferences) into fixd.yml (ConfigManager).
...
func migrateDaemonSettings(ss *preferences.SettingsService, cm *config.ConfigManager, dataDir string) {
	...
	if migrated > 0 {
		logrus.WithField("count", migrated).Info("Migrated daemon settings from settings.json to fixd.yml")
	}
