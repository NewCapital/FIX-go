...
// SendTransaction sends FIX to an address
...
// SendTransactionWithOptions sends FIX with advanced options
...
type SendTransactionMultiRequest struct {
	// Recipients maps addresses to amounts in FIX
...
// SendTransactionMulti sends FIX to multiple recipients in a single transaction
// Returns a SendTransactionResult with either txid on success or a structured SendError
...
// Returns the estimated fee in FIX, number of inputs that would be used, and tx size in bytes
