...
var (
	...
	fixAppUserModelID = "com.fix.wallet"
	fixDisplayName    = "FIX Wallet"
)
...
func configureWindowsTaskbarIdentity() {
	appID, err := syscall.UTF16PtrFromString(fixAppUserModelID)
	if err != nil {
...
// .exe resource so the Windows taskbar shows the FIX icon.
...
func applyTaskbarProperties(hwnd uintptr) {
	...
	if err := store.SetString(pkeyAppUserModelRelaunchDisplayName, fixDisplayName); err != nil {
	...
	if err := store.SetString(pkeyAppUserModelID, fixAppUserModelID); err != nil {
