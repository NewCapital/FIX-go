...
import (
	...
	binary "github.com/NewCapital/FIX-go/internal/storage/binary"
...
type MasternodeStatistics struct {
	...
	TotalCollateral int64              `json:"totalCollateral"` // In whole FIX coins (for display purposes)
...
func (a *App) GetMasternodeStatistics() (*MasternodeStatistics, error) {
	...
	// Calculate total collateral (in FIX, not satoshis)
...
type MasternodeOutput struct {
	TxHash        string  `json:"txHash"`
	OutputIndex   int     `json:"outputIndex"`
	Amount        float64 `json:"amount"`        // In FIX
...
func (a *App) GenerateMasternodeKey() (string, error) {
	...
	// Version byte 66 (0x42) for FIX mainnet, uncompressed=false (matches legacy MakeNewKey(false))
...
func (a *App) GetMasternodeOutputs() ([]MasternodeOutput, error) {
	...
	for _, utxo := range utxos {
		// Amount is in FIX (float64), convert to satoshis for tier validation
...
func (a *App) validateMasternodeEntry(entry MasternodeConfigEntry) error {
	...
	// Validate port is 17464 on mainnet
...
func getTierName(amountSatoshis int64) string {
	...
	const (
		TierBronzeCollateral   = 1_000_000 * 1e8   // 1M FIX
		TierSilverCollateral   = 5_000_000 * 1e8   // 5M FIX
		TierGoldCollateral     = 20_000_000 * 1e8  // 20M FIX
		TierPlatinumCollateral = 100_000_000 * 1e8 // 100M FIX
	)
...
type PaymentStatsResponse struct {
	TotalPaid              float64             `json:"totalPaid"`              // Total paid across all MNs in FIX
...
// Decodes scriptPubKeys to FIX addresses, cross-references against known masternodes for tier,
...
func (a *App) GetPaymentStats(filter PaymentStatsFilter) (*PaymentStatsResponse, error) {
	...
	for hexScript, stats := range allStats {
		...
		// Extract FIX address from scriptPubKey using the active network's prefix
		...
		totalPaidFIX := float64(stats.TotalPaid) / 1e8
...
// extractAddressFromScriptPubKey decodes a scriptPubKey to a FIX address string
