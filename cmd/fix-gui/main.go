...
import (
	...
	"github.com/NewCapital/FIX-go/internal/gui/constants"
...
func main() {
	// Set default log level to error (matching fixd behavior).
	// Must run before any daemon components are initialized.
	logLevel := logrus.ErrorLevel
	if envLevel := os.Getenv("FIX_LOG_LEVEL"); envLevel != "" {
	...
	err = wails.Run(&options.App{
		...
		Mac: &mac.Options{
			...
			WindowIsTranslucent:  false,
			About: &mac.AboutInfo{
				Title:   "FIX Core - Wallet",
				Message: "© 2024 FIX Development Team",
				Icon:    appIcon,
			},
		},
		Linux: &linux.Options{
