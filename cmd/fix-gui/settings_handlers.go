...
// float64ToInt64 converts a JSON float64 to int64 with overflow check.
func float64ToInt64(v float64) (int64, error) {
	if v > float64(math.MaxInt64) || v < float64(math.MinInt64) {
		return 0, fmt.Errorf("value %f out of range for int64", v)
	}
	return int64(v), nil
}
...
func (a *App) validateSettingUpdate(key string, value interface{}) error {
	...
	case float64:
		...
		return validateNumericRange(key, intVal, metadata)
	case bool:
