...
func (a *App) addMacOSMenus(appMenu *menu.Menu) {
	// App menu (FIX)
	fixMenu := appMenu.AddSubmenu("FIX")
	fixMenu.AddText("About FIX", nil, a.handleAbout)
	fixMenu.AddSeparator()
	fixMenu.AddText("Preferences...", keys.CmdOrCtrl(","), a.handlePreferences)
	fixMenu.AddSeparator()
	fixMenu.AddText("Hide FIX", keys.CmdOrCtrl("h"), nil)
	fixMenu.AddText("Hide Others", keys.Combo("h", keys.CmdOrCtrlKey, keys.OptionOrAltKey), nil)
	fixMenu.AddText("Show All", nil, nil)
	fixMenu.AddSeparator()
	fixMenu.AddText("Quit FIX", keys.CmdOrCtrl("q"), a.handleQuit)
...
func (a *App) addWindowsLinuxMenus(appMenu *menu.Menu) {
	...
	helpMenu.AddText("About FIX", nil, a.handleAbout)
}
...
func (a *App) handleAbout(_ *menu.CallbackData) {
	wailsRuntime.MessageDialog(a.ctx, wailsRuntime.MessageDialogOptions{
		...
		Title:   "About FIX Wallet",
		Message: "FIX Core Wallet\nModern Go Implementation\n\nBuilt with Wails",
	})
}
