...
// GetAddressBalances returns the total balance for each wallet address in FIX.
...
func buildReceivingAddressRows(w *wallet.Wallet, filter core.ReceivingAddressFilter) (rows []core.ReceivingAddressRow, totalAll int, err error) {
	...
	// 2) Snapshot per-address balances in FIX.
	satoshiBalances := w.GetTotalBalancesByAddress()
	balanceFIX := func(addr string) float64 {
	...
	for _, addr := range addresses {
		...
		balance := balanceFIX(addr.Address)
		...
		// (Floating-point comparison to 0 is safe here because balanceFIX
...
func (a *App) ExportReceivingAddressesCSV(filter core.ReceivingAddressFilter) (bool, error) {
	...
	// Header columns: Label, Address, Balance (FIX), Has Payment Request
	var sb strings.Builder
	sb.WriteString("\ufeff") // UTF-8 BOM for Excel compatibility
	sb.WriteString("Label,Address,Balance (FIX),Has Payment Request\n")
	for _, r := range rows {
		hasReq := "no"
		if r.HasPaymentRequest {
			hasReq = "yes"
		}
		// Format balance with 8 decimal places (FIX native precision)
