...
// the transactions slice keys (which use the same `fix_` namespace).
//
// Note: previous versions of this slice also persisted `fix_recvAddrsShowAll`
// and `fix_recvAddrsShowZeroBalance`. Those keys are intentionally left
// orphaned in existing user browsers — no migration is run. They sit unused
// and have no effect on current state.
const STORAGE_KEY_PAGE_SIZE = 'fix_recvAddrsPageSize';
const STORAGE_KEY_SORT_COLUMN = 'fix_recvAddrsSortColumn';
const STORAGE_KEY_SORT_DIRECTION = 'fix_recvAddrsSortDirection';
const STORAGE_KEY_HIDE_ZERO_BALANCE = 'fix_recvAddrsHideZeroBalance';
