...
export const GeneralTab: React.FC<GeneralTabProps> = ({ settings, metadata, themes, onChange, daemonValues, pendingDaemonChanges, onDaemonChange }) => {
  ...
  const [migrationDismissed, setMigrationDismissed] = useState(() => {
    try { return localStorage.getItem('fix_autocombine_migration_dismissed') === '1'; } catch { return false; }
  });
    ...
    <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '12px' }}>
          ...
          >
            <option value={0}>{t('display.unitFix')}</option>
          ...
          <div style={{
            ...
            <button
              onClick={() => {
                ...
                try { localStorage.setItem('fix_autocombine_migration_dismissed', '1'); } catch { /* ignore */ }
              }}
              style={{
          ...
          {autoCombineEnabled && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
                ...
                />
                <span style={{ color: '#888', fontSize: '13px', marginLeft: '8px' }}>FIX</span>
              </div>
