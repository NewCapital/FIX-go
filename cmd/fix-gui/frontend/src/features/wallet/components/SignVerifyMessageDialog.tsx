...
export const SignVerifyMessageDialog: React.FC = () => {
  ...
  const handleSign = useCallback(async () => {
    ...
    if (!signAddress.trim()) {
      setSignError('Please enter a FIX address.');
      return;
    }
    if (!signMessage.trim()) {
  ...
  const handleVerify = useCallback(async () => {
    ...
    if (!verifyAddress.trim()) {
      setVerifyError('Please enter a FIX address.');
      return;
    }
...
const SignTab: React.FC<SignTabProps> = ({
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
    ...
    {/* Address Field */}
    <div>
      <label style={labelStyle}>FIX Address</label>
      <div style={{ position: 'relative' }} ref={dropdownRef}>
        <div style={{ display: 'flex' }}>
          <input
            ...
            placeholder="Enter a FIX address (e.g., W...)"
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
    ...
    <div>
      ...
      <input
        ...
        value={address}
        onChange={(e) => onAddressChange(e.target.value)}
        placeholder="Enter the signer's FIX address"
        style={inputStyle}
      />
    </div>
