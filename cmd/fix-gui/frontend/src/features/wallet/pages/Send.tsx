...
const recipientSchema = z.object({
  address: z.string()
    ...
    .regex(FIX_ADDRESS_REGEX, 'Invalid FIX address format'),
  amount: z.string()
...
export const Send: React.FC = () => {
  ...
  const handleUseMaximum = (recipientIndex: number) => {
    ...
    const otherRecipientsSatoshis = watchedRecipients.reduce((sum, recipient, index) => {
      ...
      const amount = parseAmount(recipient.amount || '0');
      return sum + (amount * SATOSHIS_PER_FIX);
    }, 0);

    const otherRecipientsTotal = otherRecipientsSatoshis / SATOSHIS_PER_FIX;
