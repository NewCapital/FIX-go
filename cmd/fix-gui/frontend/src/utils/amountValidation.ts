/**
 * Amount validation utilities for FIX wallet
 * Handles FIX precision (8 decimal places) and validation rules
 */

import { addThousandsSeparators } from '@/shared/utils/format';

// Constants
export const FIX_PRECISION = 8;
export const SATOSHIS_PER_FIX = 100_000_000;
export const MAX_FIX = 100_000_000_000; // 100 billion FIX
export const MIN_FIX = 0.00000001; // 1 satoshi
...
export const UNIT_FACTORS = {
  [AmountUnit.FIX]: 1,
  [AmountUnit.mFIX]: 1000,
  [AmountUnit.uFIX]: 1_000_000
};
...
export const formatAmountInput = (value: string): string => {
  ...
  // Limit decimal places to FIX precision
  if (parts.length === 2 && parts[1].length > FIX_PRECISION) {
    formatted = parts[0] + '.' + parts[1].substring(0, FIX_PRECISION);
  }
...
/**
 * Parse amount string to number (in FIX)
 *
...
export const parseAmount = (value: string): number => {
  ...
  // Round to FIX precision to avoid floating point issues
  // This converts to satoshis (integer) then back to FIX
  return Math.round(parsed * SATOSHIS_PER_FIX) / SATOSHIS_PER_FIX;
};

/**
 * Convert amount to satoshis (smallest unit)
 */
export const toSatoshis = (fix: number): number => {
  return Math.round(fix * SATOSHIS_PER_FIX);
};
...
export const fromSatoshis = (satoshis: number): number => {
  return satoshis / SATOSHIS_PER_FIX;
};
...
export const formatAmountForInput = (amount: number): string => {
  ...
  const fixed = amount.toFixed(FIX_PRECISION);
 ...
 *
 * @param amount - The amount to format
 * @param showUnit - Whether to include "FIX" suffix (default: true)
...
export const formatAmountDisplay = (
  ...
  if (amount === 0) return showUnit ? '0 FIX' : '0';
  ...
  if (trimZeros && parts[1]) {
    parts[1] = trimTrailingZeros(parts[1]);
    if (parts[1].length === 0) {
      return showUnit ? `${parts[0]} FIX` : parts[0];
    }
  }

  const formatted = parts.join('.');
  return showUnit ? `${formatted} FIX` : formatted;
};
...
export const convertUnit = (amount: number, fromUnit: AmountUnit, toUnit: AmountUnit): number => {
  const inFix = amount / UNIT_FACTORS[fromUnit];
  return inFix * UNIT_FACTORS[toUnit];
};
...
export const validateAmount = (
  ...
  if (value < DUST_THRESHOLD) {
    return {
      isValid: false,
      error: `Amount below minimum transaction size (${DUST_THRESHOLD} FIX)`
    };
  }
  ...
  if (value > MAX_FIX) {
    return {
      isValid: false,
      error: `Amount exceeds maximum (${formatAmountDisplay(MAX_FIX)})`
    };
  }
...
export const validateMultipleAmounts = (
  ...
  for (const amount of amounts) {
    ...
    if (value < DUST_THRESHOLD && value > 0) {
      return {
        isValid: false,
        error: `One or more amounts below minimum (${DUST_THRESHOLD} FIX)`
      };
    }
...
export const formatForBackend = (amount: number): string => {
  return amount.toFixed(FIX_PRECISION);
};
