// Formatting utilities for the FIX wallet

// Display unit constants matching backend preferences.DisplayUnit*
export const DISPLAY_UNIT_FIX = 0;
export const DISPLAY_UNIT_MFIX = 1;
export const DISPLAY_UNIT_UFIX = 2;

/**
 * Convert an amount from base FIX to the selected display unit.
 * 1 FIX = 1,000 mFIX = 1,000,000 uFIX
 */
export function convertToDisplayUnit(amount: number, displayUnit: number): number {
  switch (displayUnit) {
    case DISPLAY_UNIT_MFIX:
      return amount * 1000;
    case DISPLAY_UNIT_UFIX:
 ...
 * @param includeUnit - Whether to include "FIX" suffix (default: true)
 */
export function formatBalance(
  ...
  return includeUnit ? `${result} FIX` : result;
}
