...
// mainnet behavior by checking the leading 'F' prefix expected from
// MainNetPubKeyHashAddrID = 0x23. If this character ever changes, every
...
func TestExtractAddressFromScriptPubKey_MainnetRegression(t *testing.T) {
	...
	if !strings.HasPrefix(got, "F") {
		t.Errorf("mainnet P2PKH address should start with 'F', got %q", got)
	}
}
