...
import (
	...
	"github.com/NewCapital/FIX-go/internal/cli"
	"github.com/NewCapital/FIX-go/internal/config"
	"github.com/NewCapital/FIX-go/internal/daemon"
	configservice "github.com/NewCapital/FIX-go/internal/gui/config"
	"github.com/NewCapital/FIX-go/internal/gui/core"
	"github.com/NewCapital/FIX-go/internal/gui/initialization"
	"github.com/NewCapital/FIX-go/internal/gui/preferences"
	"github.com/NewCapital/FIX-go/internal/gui/shutdown"
	"github.com/NewCapital/FIX-go/internal/gui/tests/mocks"
	"github.com/NewCapital/FIX-go/internal/gui/window"
...
const (
	defaultP2PListenAddr = "0.0.0.0:17464"      // Standard FIX P2P port
	defaultP2PMaxPeers   = 125                  // Max P2P connections
	defaultRPCPort       = types.DefaultRPCPort // Standard FIX RPC port
)
...
type App struct {
	...
	rpcServer         *rpc.Server                    // RPC server for fix-cli compatibility
	rpcStarted        atomic.Bool                    // Prevents multiple RPC server starts
	initService       *initialization.Service
	configService     *configservice.Service
	configManager     *config.ConfigManager // Unified daemon config (fixd.yml)
...
func (a *App) startup(ctx context.Context) {
	a.ctx = ctx
	fmt.Println("FIX Wallet starting...")
	...
	// Log settings and CLI config if -dev-fulllogs flag or FIX_DEV_FULL_LOGS env var is set
	showFullLogs := (a.guiConfig != nil && a.guiConfig.DevFullLogs) || os.Getenv("FIX_DEV_FULL_LOGS") == "1"
...
func (a *App) initializeFullDaemon() {
	...
	// Create ConfigManager — unified daemon config authority (fixd.yml).
	// Loads defaults → YAML → env var overrides.
	yamlPath := filepath.Join(a.dataDir, "fixd.yml")
	...
	// One-time migration: daemon settings from settings.json → fixd.yml
	if a.settingsService != nil {
	...
	// Guard against FIX_LOG_LEVEL: main.go reads that env var before ConfigManager
	// exists and applies it directly to logrus. Preserve that user-supplied value here.
	// (ConfigManager uses FIX_LOGGING_LEVEL, which it locks internally via cliLocks.)
	if os.Getenv("FIX_LOG_LEVEL") == "" {
	...
	node, err := startup.Start(a.ctx, startup.Config{
		...
		WalletConfig: daemon.WalletConfig{
			FullConfig: configSnapshot, // applies wallet settings from fixd.yml (fees, keypool, reserveBalance, etc.)
			UseTxCache: true,
		},
		P2PConfig:     p2pCfg,
		Staking:       staking,
	...
	if node.Wallet != nil {
		...
		node.Wallet.SetOnConsolidationCallback(func(txCount int, totalAmount int64) {
			amountFIX := float64(totalAmount) / 100_000_000
...
func (a *App) shutdown(ctx context.Context) {
	fmt.Println("FIX Wallet shutting down...")
...
func (a *App) StartP2P() error {
	...
	// Start RPC server for fix-cli compatibility (non-blocking)
	go func() {
...
// StartRPCServer initializes and starts the RPC server for fix-cli compatibility.
...
// Persists to fixd.yml via ConfigManager and triggers the staking subscriber
...
func (a *App) ToggleStaking(enabled bool) map[string]interface{} {
	...
	if cm := a.getConfigManager(); cm != nil {
		// ConfigManager.Set persists to fixd.yml + triggers subscriber (start/stop staking)
	...
	// no ConfigManager to write fixd.yml. Log a warning so we notice if it
