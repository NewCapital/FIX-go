...
func main() {
	app := clilib.CreateBaseApp("fix-cli", "FIX cryptocurrency RPC client", clilib.String())

	// Add RPC client flags
	app.Flags = append(app.Flags, clilib.CommonRPCClientFlags()...)
...
// fixdYAMLConfig is a minimal struct to parse RPC settings from fixd.yml
type fixdYAMLConfig struct {
	RPC struct {
...
// parseFixdYAML reads all RPC settings from a YAML config file in one pass.
func parseFixdYAML(yamlPath string) (*fixdYAMLConfig, error) {
	data, err := os.ReadFile(yamlPath)
	if err != nil {
		return nil, err
	}

	var cfg fixdYAMLConfig
...
func parseYAMLConfig(yamlPath string) (username, password string, err error) {
	cfg, err := parseFixdYAML(yamlPath)
	if err != nil {
		return "", "", err
	}
...
func getRPCCredentials(c *cli.Context) (username, password string) {
	...
	// before the subcommand name (e.g., "fix-cli --rpc-user=x getinfo").
	username = clilib.GetStringFromLineage(c, "rpc-user")
	password = clilib.GetStringFromLineage(c, "rpc-password")
	if username != "" && password != "" {
		return username, password
	}

	// 2. Use shared config discovery logic (same as fixd)
	configPath, explicitConfig := clilib.GetEffectiveConfigPath(c)
...
func getRPCEndpoint(c *cli.Context) (host string, port int) {
	...
	if clilib.IsSetInLineage(c, "rpc-host") {
		host = clilib.GetStringFromLineage(c, "rpc-host")
	}
	if clilib.IsSetInLineage(c, "rpc-port") {
		port = clilib.GetIntFromLineage(c, "rpc-port")
	}
