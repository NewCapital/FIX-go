...
func (pd *PeerDiscovery) loadSeedAddresses() {
	for _, seed := range pd.seeds {
		...
		for _, ip := range ips {
			// Parse port - use FIX network-specific default
...
// Replaces hardcoded Bitcoin port 18333 with FIX network-specific ports
func (pd *PeerDiscovery) getDefaultPort() int {
	switch pd.network {
	case "mainnet":
		return 17464 // FIX mainnet port
	case "testnet":
		return 5447 // FIX testnet port (legacy chainparams.cpp:370)
	case "regtest":
		return 5467 // FIX regtest port (legacy chainparams.cpp:494)
	default:
		return 17464 // Default to mainnet
	}
}
...
// FIX legacy/src/chainparams.cpp: mainnet lines 233-243, testnet lines 362-370.
