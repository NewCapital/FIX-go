...
func TestWhitelistManager_IsWhitelistedAddr(t *testing.T) {
	...
	tcpAddr := &net.TCPAddr{IP: net.ParseIP("192.168.1.50"), Port: 17464}
	assert.True(t, wm.IsWhitelistedAddr(tcpAddr))

	// TCP address not in whitelist
	tcpAddr2 := &net.TCPAddr{IP: net.ParseIP("10.0.0.1"), Port: 17464}
...
func TestIsOnionAddress(t *testing.T) {
	...
	assert.True(t, IsOnionAddress("abcdefghij1234567890.onion:17464"))

	// Regular addresses
	assert.False(t, IsOnionAddress("192.168.1.1:17464"))
	assert.False(t, IsOnionAddress("example.com:17464"))
	assert.False(t, IsOnionAddress("onion.example.com:17464"))
}
