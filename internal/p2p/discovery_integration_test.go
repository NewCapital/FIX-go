...
func TestWarmStartBootstrap(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		addr := createIntegrationAddress(fmt.Sprintf("192.168.%d.%d", i/256, i%256), 17464)
		addresses[i] = addr
...
func TestPeerDiversification(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		addr := createIntegrationAddress(fmt.Sprintf("192.168.1.%d", i+1), 17464)
		known := &KnownAddress{
			Addr:     addr,
			LastSeen: time.Now(),
			Services: SFNodeNetwork,
	...
	for i := 0; i < 50; i++ {
		addr := createIntegrationAddress(fmt.Sprintf("10.%d.1.1", i), 17464)
		known := &KnownAddress{
			Addr:     addr,
			LastSeen: time.Now(),
			Services: SFNodeNetwork,
...
func TestFailedPeerEviction(t *testing.T) {
	...
	// Add an address
	addr := createIntegrationAddress("192.168.1.100", 17464)
	known := &KnownAddress{
		Addr:     addr,
		LastSeen: time.Now(),
		Services: SFNodeNetwork,
...
func TestStaleAddressRemoval(t *testing.T) {
	...
	oldAddr := createIntegrationAddress("192.168.1.100", 17464)
	...
	recentAddr := createIntegrationAddress("192.168.1.101", 17464)
...
func TestPermanentAddressProtection(t *testing.T) {
	...
	permAddr := createIntegrationAddress("192.168.1.100", 17464)
	...
	tempAddr := createIntegrationAddress("192.168.1.101", 17464)
...
func TestAddressSourceTracking(t *testing.T) {
	...
	sourceAddr := createIntegrationAddress("10.0.1.1", 17464)

	addr1 := createIntegrationAddress("192.168.1.100", 17464)
	known1 := &KnownAddress{
		Addr:     addr1,
		LastSeen: time.Now(),
		Services: SFNodeNetwork,
	...
	addr2 := createIntegrationAddress("192.168.1.101", 17464)
	known2 := &KnownAddress{
		Addr:     addr2,
		LastSeen: time.Now(),
		Services: SFNodeNetwork,
...
func TestMasternodePrioritization(t *testing.T) {
	...
	// Add masternodes
	for i := 0; i < 10; i++ {
		addr := createIntegrationAddress(fmt.Sprintf("10.0.1.%d", i+1), 17464)
		known := &KnownAddress{
			Addr:        addr,
			LastSeen:    time.Now(),
			LastSuccess: time.Now().Add(-1 * time.Hour),
