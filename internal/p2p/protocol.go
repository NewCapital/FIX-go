...
// Protocol constants
const (
	// Protocol version (must match legacy FIX)
	ProtocolVersion = 70928

	// MinPeerProtocol is the minimum accepted peer protocol version.
	// Peers below this are disconnected. Matches legacy FIX mainnet PROTOCOL_VERSION
	// (legacy/src/version.h:15) and masternode.MinPeerProtoBeforeEnforcement so the
	// daemon can bootstrap against legacy nodes still running proto 70926.
	MinPeerProtocol = 70926

	// Network magic bytes (FIX-specific, NOT Bitcoin).
	MainNetMagic = 0x742c4d64 // FIX mainnet: 0x74, 0x2c, 0x4d, 0x64 (legacy chainparams.cpp:157-160)
	TestNetMagic = 0x446aa4cc // FIX testnet: 0x44, 0x6a, 0xa4, 0xcc (legacy chainparams.cpp:308-311)
	RegTestMagic = 0x2afcc7ca // FIX regtest: 0x2a, 0xfc, 0xc7, 0xca (legacy chainparams.cpp:421-424)
...
// Message represents a FIX protocol message
type Message struct {
...
const (
	...
	SFNodeGetUTXO        ServiceFlag = 1 << 1 // NODE_GETUTXO - UTXO lookup service (not used in FIX)
	SFNodeBloom          ServiceFlag = 1 << 2 // NODE_BLOOM - Bloom filter service (SPV support)
	SFNodeWitness        ServiceFlag = 1 << 3 // NODE_WITNESS - Segregated witness support (not used in FIX)
	SFNodeBloomWithoutMN ServiceFlag = 1 << 4 // NODE_BLOOM_WITHOUT_MN - Bloom filter but no masternode messages
	SFNodeMasternode     ServiceFlag = 1 << 5 // NODE_MASTERNODE - Masternode capable node (FIX specific)
