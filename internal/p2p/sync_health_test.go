...
func TestPruneDisconnectedPeers(t *testing.T) {
	...
	// Register 5 peers
	tracker.RecordPeerDiscovered("peer1:17464", 1000, false, TierNone, true)
	tracker.RecordPeerDiscovered("peer2:17464", 1000, false, TierNone, true)
	tracker.RecordPeerDiscovered("peer3:17464", 1000, false, TierNone, false)
	...
	for _, addr := range []string{"peer1:17464", "peer3:17464", "peer5:54321"} {
		if _, exists := allPeers[addr]; !exists {
			t.Errorf("expected peer %s to remain after prune", addr)
		}
	}
	for _, addr := range []string{"peer2:17464", "peer4:12345"} {
		if _, exists := allPeers[addr]; exists {
