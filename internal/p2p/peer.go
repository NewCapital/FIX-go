...
// ErrBadMagic indicates the remote sent bytes that don't match any FIX network magic.
...
// isNonProtocolError returns true for errors caused by non-FIX connections
...
// Peer represents a connected peer in the FIX network
type Peer struct {
	// Connection details
	conn        net.Conn
	addr        *NetAddress
...
func (p *Peer) readMessage() (*Message, error) {
	...
	// Rejects non-FIX connections (HTTP probes, TLS scanners, etc.) immediately
...
// Legacy: implements FIX Misbehaving() function from main.cpp
