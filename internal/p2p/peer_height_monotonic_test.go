...
func TestRecordPeerDiscovered_TipHeightMonotonic(t *testing.T) {
	...
	// Live advance: a ping (proto 70928), inv, or headers message announces height
	// 1_490_182, cascading through UpdateBestKnownHeight from the handler layer.
...
// monotonic guard still allows forward motion when a re-discovery actually
// brings a higher value (defensive — shouldn't happen for live peers, but the
