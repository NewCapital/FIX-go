...
func TestMarkAttempt(t *testing.T) {
	...
	addr := createTestAddress("192.168.1.100:17464")
...
func TestMarkFailureThreshold(t *testing.T) {
	...
	addr := createTestAddress("192.168.1.1:17464")
...
func TestKnownAddressClone(t *testing.T) {
	addr := createTestAddress("192.168.1.1:17464")
	source := createTestAddress("192.168.1.100:17464")
	...
	assert.Equal(t, uint16(17464), original.Addr.Port)
...
func TestAddressPersistence(t *testing.T) {
	...
	addr1 := createTestAddress("192.168.1.100:17464")
	addr2 := createTestAddress("192.168.1.101:17464")
...
func TestPriorityCalculation(t *testing.T) {
	...
	known2 := &KnownAddress{
		Addr:     createTestAddress("192.168.1.101:17464"),
	...
	known3 := &KnownAddress{
		Addr:        createTestAddress("192.168.1.102:17464"),
		LastSuccess: time.Now().Add(-1 * time.Hour),
		LastSeen:    time.Now(),
		Services:    SFNodeNetwork | SFNodeMasternode,
		Attempts:    1,
...
func TestCleanupAddresses(t *testing.T) {
	...
	oldAddr := createTestAddress("192.168.1.100:17464")
	known1 := &KnownAddress{
		Addr:     oldAddr,
	...
	badAddr := createTestAddress("192.168.1.101:17464")
	known2 := &KnownAddress{
		Addr:     badAddr,
		LastSeen: time.Now(),
		Services: SFNodeNetwork,
	...
	goodAddr := createTestAddress("192.168.1.102:17464")
	known3 := &KnownAddress{
		Addr:        goodAddr,
		LastSeen:    time.Now(),
		LastSuccess: time.Now().Add(-1 * time.Hour),
	...
	permAddr := createTestAddress("192.168.1.103:17464")
	known4 := &KnownAddress{
		Addr:      permAddr,
		LastSeen:  time.Now().Add(-30 * 24 * time.Hour), // 30 days old
		Services:  SFNodeNetwork,
...
func TestGetAddresses(t *testing.T) {
	...
	for i := 1; i <= 10; i++ {
		addr := createTestAddress(fmt.Sprintf("192.168.1.%d:17464", i))
		known := &KnownAddress{
			Addr:     addr,
			LastSeen: time.Now(),
			Services: SFNodeNetwork,
...
func TestWeightedSelection(t *testing.T) {
	...
	// Masternode address
	addr3 := createTestAddress("192.168.1.102:17464")
...
func TestCalculateChance(t *testing.T) {
	...
	regularNode := &KnownAddress{
		Addr:        createTestAddress("192.168.1.103:17464"),
		LastAttempt: time.Now().Add(-1 * time.Hour),
		LastSeen:    time.Now(),
		Services:    SFNodeNetwork,
		Attempts:    0,
