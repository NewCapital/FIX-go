...
func (w *Wallet) categorizeTransactionLocked(
	...
	if isCoinbase {
		...
		// to a position-based check. In FIX PoS coinstakes the canonical output
		// layout is:
		//
		//   [output[0]=empty, stake_return..., mn_payment, dev_payment]
		//
		// The MN payment sits at structural position len-2 because FIX always
		...
		// >= 4 (the minimum FIX coinstake layout: empty + stake + mn + dev). The
		...
		// structural Pass 2 resolves that ambiguity by using the canonical FIX
