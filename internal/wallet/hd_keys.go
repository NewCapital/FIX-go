...
const (
	...
	// FIXCoinType is the FIX coin type in BIP44
	FIXCoinType = 970 + HardenedKeyStart // Using 970 as FIX coin type (from chainparams.cpp:311)
)
...
func (k *HDKey) GetAddress() (string, error) {
	...
	// Add version byte (FIX mainnet: 0x49, testnet: 0x6f)
	var version byte
	switch k.network {
	case MainNet:
...
func addPrivateKeys(baseKey *crypto.PrivateKey, addition []byte) (*crypto.PrivateKey, error) {
	...
	// Bitcoin/FIX use secp256k1, not NIST P-256
