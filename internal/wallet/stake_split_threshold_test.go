...
// hard floor MinStakeSplitThresholdSatoshis (100000 FIX) are clamped up to
// the floor, preventing the wallet from producing split coinstakes whose vout[1]
// would fall below legacy's StakingMinInput (12000 FIX). See task
...
func TestSetStakeSplitThresholdClampsBelowMinimum(t *testing.T) {
	...
	// Try to set a value well below the hard floor (50000 FIX).
