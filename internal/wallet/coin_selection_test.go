...
func TestCoinSelectionKnapsack(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			utxoValues:  []int64{100000000, 200000000, 500000000}, // 1, 2, 5 FIX
			target:      200000000,                                // 2 FIX
			wantSuccess: true,
		},
		{
			name:        "need subset of coins",
			utxoValues:  []int64{100000000, 200000000, 300000000, 400000000}, // 1,2,3,4 FIX
			target:      500000000,                                           // 5 FIX
			wantSuccess: true,
		},
		{
			name:        "many small UTXOs",
			utxoValues:  []int64{10000000, 10000000, 10000000, 10000000, 10000000, 10000000, 10000000, 10000000, 10000000, 10000000}, // 10x 0.1 FIX
			target:      50000000,                                                                                                    // 0.5 FIX
			wantSuccess: true,
		},
		{
			name:        "insufficient funds",
			utxoValues:  []int64{100000000, 200000000}, // 1, 2 FIX
			target:      500000000,                     // 5 FIX
			wantSuccess: false,
		},
		{
			name:        "single large coin for small target",
			utxoValues:  []int64{1000000000}, // 10 FIX
			target:      100000000,           // 1 FIX
			wantSuccess: true,
		},
		{
			name:        "prefer fewer inputs over many small",
			utxoValues:  []int64{50000000, 50000000, 50000000, 50000000, 300000000}, // 4x0.5 + 3 FIX
			target:      200000000,                                                  // 2 FIX
			wantSuccess: true,
		},
		{
			name:        "zero-conf change UTXO in tier 3",
			utxoValues:  []int64{500000000}, // 5 FIX (will be zero-conf change)
			target:      100000000,          // 1 FIX
			wantSuccess: true,
		},
	}
...
func TestCoinSelection3TierFallback(t *testing.T) {
	...
	addTestUTXO(w, makeOutpoint(0x01, 0), 50000000, 900) // 0.5 FIX

	// UTXO with 3 confs (height 998) - NOT eligible tier 1, eligible tier 2 (1,1)
	addTestUTXO(w, makeOutpoint(0x02, 0), 200000000, 998) // 2 FIX
	...
	// Test tier 1: target 0.3 FIX - only 0.5 UTXO (101 confs) is eligible at tier 1
	...
	// Test tier 2: target 2 FIX - needs the 3-conf UTXO (falls through to tier 2)
	selected, err = w.SelectUTXOs(200000000, 6, 2)
	require.NoError(t, err)
	t.Logf("  Tier 2 (target 2.0): %d UTXOs selected, total=%d\n", len(selected), sumUTXOs(selected))

	// Test tier 3: target 5 FIX - needs all including zero-conf change
...
func TestTxSizePreEstimateCheck(t *testing.T) {
	...
	recipients := map[string]int64{
		"DTestAddr1111111111111111111111": 50000000, // 0.5 FIX
	}
...
func TestCoinSelectionSizeAwareFallback(t *testing.T) {
	...
	w.mu.Lock()
	// Create 1500 small UTXOs (each 100,000 sat = 0.001 FIX)
	// Total: 150,000,000 sat = 1.5 FIX
	...
	// Create 2 large UTXOs (each 500,000,000 sat = 5 FIX)
	...
	t.Logf("  Target:     100000000 satoshis (1.0 FIX)\n")
	t.Logf("  Selected:   %d UTXOs\n", len(selected))
	t.Logf("  Total sel:  %d satoshis (%.8f FIX)\n", totalSelected, float64(totalSelected)/1e8)
