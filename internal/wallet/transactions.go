...
const (
	...
	// MinRelayFee is the minimum fee to relay transaction (1000 satoshis = 0.00001 FIX)
	MinRelayFee = 1000
	...
	// CentThreshold is 0.01 FIX in satoshis, used by knapsack for change avoidance.
...
// SendToAddress sends FIX to a single address
...
// SendMany sends FIX to multiple addresses in one transaction
...
// SendManyWithOptions sends FIX to multiple addresses with advanced options
...
func (w *Wallet) ListUnspent(minConf, maxConf int, addresses []string) (interface{}, error) {
	...
	for _, utxo := range w.utxos {
		...
		result = append(result, &UnspentOutput{
			...
			Amount:        float64(utxo.Output.Value) / 100000000, // Convert to FIX
...
func (w *Wallet) SelectUTXOs(targetAmount int64, minConf int, numOutputs int) ([]*UTXO, error) {
	...
	if len(available) == 0 {
		...
		if collateralAmount > 0 {
			lockedFIX := float64(collateralAmount) / 1e8
			return nil, fmt.Errorf("insufficient unlocked funds. You have %.8f FIX locked as masternode collateral in %d UTXOs (total=%d, not_spendable=%d)",
				lockedFIX, collateralCount, len(w.utxos), notSpendableCount)
		}
...
func (w *Wallet) calculateSignatureHash(tx *types.Transaction, inputIndex int, scriptPubKey []byte) ([]byte, error) {
	...
	// This ensures Bitcoin/FIX protocol compatibility with proper:
...
// ValidateAddress validates a FIX address and returns information about it
