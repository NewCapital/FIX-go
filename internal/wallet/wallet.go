...
import (
	...
	walletcrypto "github.com/NewCapital/FIX-go/internal/wallet/crypto"
	"github.com/NewCapital/FIX-go/internal/wallet/legacy"
...
func DefaultConfig() *Config {
	return &Config{
		...
		MinConfirmations:    6,
		FeePerKB:            10000, // 0.0001 FIX per KB (legacy: -paytxfee default)
		EncryptWallet:       false,
		...
		MaxTxFee:            100000000,  // 1 FIX (legacy: -maxtxfee default)
		...
		// FIX legacy chainparams.cpp:186 sets nStakeMinInput = 100 * COIN.
		// NOTE: `internal/daemon/node_wallet.go` overwrites this with chainParams.MinStakeAmount
		// during InitWallet — this default is used only when wallet is constructed without
		// daemon wiring (e.g. tests).
		MinStakeAmount: 10000000000, // 100 FIX in satoshis (100 * 100000000)
	}
}
...
func (w *Wallet) LoadWallet() error {
	...
	if err != nil {
		...
		for _, pubkeyBytes := range pubkeys {
			...
			case MainNet:
				version = crypto.MainNetPubKeyHashAddrID // 0x49 - FIX W... addresses
...
func (w *Wallet) dumpPrivKeyLocked(address string) (string, error) {
	...
	switch w.config.Network {
	case MainNet:
		netID = crypto.PrivateKeyID // 0x42 for FIX MainNet
	case TestNet, RegTest:
...
func (w *Wallet) deriveHDPrivKeyLocked(pubkey []byte) (*crypto.PrivateKey, error) {
	...
	// FIX coin type is 970
	path := &KeyPath{
		CoinType: 970, // FIX ExtCoinType
...
func (w *Wallet) SetStakeSplitThreshold(threshold int64) error {
	...
	// FIX), which legacy nodes reject at CheckBlock. Threshold 0 remains a
...
func (w *Wallet) SetTransactionFee(feePerKB int64) error {
	...
	// Maximum fee: 1 FIX per KB (100,000,000 satoshis)
	const maxFee int64 = 100000000
	if feePerKB > maxFee {
		return fmt.Errorf("fee exceeds maximum allowed (1 FIX/kB)")
	}
...
func (w *Wallet) SetMaxTxFee(fee int64) error {
	...
	const maxAllowed int64 = 1_000_000_000 // 10 FIX — matches registry validation upper bound
	if fee > maxAllowed {
...
func (w *Wallet) DumpWallet(filename string) error {
	...
	// Write header
	dumpTime := time.Now()
	fmt.Fprintf(file, "# Wallet dump created by FIX %s\n", dumpTime.Format(time.RFC3339))
...
func (w *Wallet) ImportAddress(address string, label string, rescan bool) error {
	...
	if !validation.IsValid {
		return fmt.Errorf("invalid FIX address")
	}
