...
func TestCreateCoinstakeTxHardFloorNoSplitBelow100K(t *testing.T) {
	...
	require.NoError(t, w.wdb.SetStakeSplitThreshold(int64(1*100000000))) // 1 FIX

	// totalReward = stakeAmount + blockReward = 99999 FIX  (< hard floor 100000)
...
func TestCreateCoinstakeTxSplitsAboveHardFloor(t *testing.T) {
	...
	// totalReward = 300000 FIX, well above the floor, and
	...
	assert.Equal(t, int64(0), tx.Outputs[0].Value, "vout[0] is empty marker")
	// Both halves must be safely above FIX StakingMinInput (100 FIX).
	const minStakeInputSatoshis = int64(100 * 100000000)
	assert.Greater(t, tx.Outputs[1].Value, minStakeInputSatoshis,
		"vout[1] must exceed MinStakeAmount")
	assert.Greater(t, tx.Outputs[2].Value, minStakeInputSatoshis,
		"vout[2] must exceed MinStakeAmount")
}
