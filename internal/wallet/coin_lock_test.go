...
func TestSelectUTXOs_SkipsUserLockedCoins(t *testing.T) {
	...
	w.mu.Lock()
	addTestUTXO(w, lockedOp, 500000000, 100) // 5 FIX - will be locked
	addTestUTXO(w, freeOp, 300000000, 100)   // 3 FIX - free
	w.mu.Unlock()
	...
	selected, err := w.SelectUTXOs(100000000, 1, 2) // need 1 FIX
	require.NoError(t, err)
