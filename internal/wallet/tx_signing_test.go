...
// extractSigAndPubkey decodes a P2PKH scriptSig: <push_len><sig+hashType><push_len><pubkey>.
...
func extractSigFromP2PKScriptSig(t *testing.T, scriptSig []byte) []byte {
	...
	sigWithHashType := scriptSig[1 : 1+sigLen]
	require.Greater(t, len(sigWithHashType), 0, "empty signature")
	return sigWithHashType[:len(sigWithHashType)-1] // strip sighash byte
}

// parsePubKey wraps btcec.ParsePubKey for the tests.
...
// TestSignCoinstakeTransaction_P2PKH_VerifiesViaStrictDER exercises the
// refactored coinstake signing path (now routed through Transaction.SignatureHash
// instead of the old custom serializeTransactionForSigning) and verifies the
// resulting signature with btcec's STRICT DER parse + verify.
//
// Legacy C++ CPubKey::Verify (legacy/src/pubkey.cpp:170-186) uses lax DER +
// secp256k1_ecdsa_signature_normalize + secp256k1_ecdsa_verify, which is
// strictly MORE PERMISSIVE than btcec's strict path. If this test passes,
// legacy accepts the signature.
...
// TestSignCoinstakeTransaction_P2PK_VerifiesViaStrictDER covers the variant where
// the UTXO being spent is already P2PK (scriptSig is just <sig>).
