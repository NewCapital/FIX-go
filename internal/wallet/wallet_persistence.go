...
import (
	...
	"github.com/NewCapital/FIX-go/internal/wallet/crypto"
	"github.com/NewCapital/FIX-go/internal/wallet/legacy"
	"github.com/NewCapital/FIX-go/internal/wallet/serialization"
	walletstorage "github.com/NewCapital/FIX-go/internal/wallet/storage"
	pkgcrypto "github.com/NewCapital/FIX-go/pkg/crypto"
	"go.etcd.io/bbolt"
)
...
func (wdb *WalletDB) GetStakeSplitThreshold() (int64, error) {
	...
	err := wdb.db.View(func(tx *bbolt.Tx) error {
		...
		if len(data) >= 8 {
			...
			// Default threshold: 20000 FIX (legacy: wallet.h:349 nStakeSplitThreshold = 20000)
			threshold = 20000 * 1e8
		}
		return nil
	})
