...
// for FIX cryptocurrency. It provides:
//
...
// Startup Sequence:
//
// ChainValidator runs during daemon startup (cmd/fixd/startup_improved.go:106-121).
...
import (
	...
	binaryStorage "github.com/NewCapital/FIX-go/internal/storage/binary"
...
// GetBlockReward returns the block reward for the given height, using the
// bootstrap-phase boundary appropriate for this chain's network.
func (bc *BlockChain) GetBlockReward(height uint32) int64 {
	var params *types.ChainParams
	if bc.config != nil {
		params = bc.config.ChainParams
	}
	return blockreward.ForParams(height, params)
}

// GetMaxBlockFees returns the maximum fees allowed in a block
func (bc *BlockChain) GetMaxBlockFees() int64 {
	const COIN = 100000000         // 1 FIX = 100,000,000 satoshis
	const maxBlockFeeFIX = 1000000 // 1M FIX cap
	return int64(maxBlockFeeFIX) * COIN
}
