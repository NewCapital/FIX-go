...
func TestCollectorEmit(t *testing.T) {
	...
	c.EmitBroadcast(TypeBroadcastReceived, "192.168.1.1:17464", "received MNB", map[string]any{
		"outpoint": "abc:0",
		"tier":     "Gold",
	})
...
func TestCollectorQuery(t *testing.T) {
	...
	c.Emit(Event{Timestamp: now.Add(-2 * time.Second), Type: TypeBroadcastReceived, Category: CategoryBroadcast, Source: "192.168.1.1:17464", Summary: "received MNB for gold tier"})
	c.Emit(Event{Timestamp: now.Add(-1 * time.Second), Type: TypePingStageResult, Category: CategoryPing, Source: "10.0.0.1:17464", Summary: "ping accepted"})
...
func TestCollectorEventPayload(t *testing.T) {
	...
	c.EmitBroadcast(TypeBroadcastAccepted, "10.0.0.1:17464", "accepted MNB", payload)
...
func TestCollectorSummary(t *testing.T) {
	...
	c.EmitBroadcast(TypeBroadcastReceived, "10.0.0.1:17464", "received MNB", map[string]any{
		"outpoint": "aaa:0", "tier": "Gold",
	})
	c.EmitBroadcast(TypeBroadcastAccepted, "10.0.0.1:17464", "accepted MNB", map[string]any{
		"outpoint": "aaa:0", "tier": "Gold",
	})
	c.EmitBroadcast(TypeBroadcastReceived, "10.0.0.2:17464", "received MNB", map[string]any{
		"outpoint": "bbb:0", "tier": "Silver",
	})
	c.EmitBroadcast(TypeBroadcastRejected, "10.0.0.2:17464", "rejected MNB", map[string]any{
		"outpoint": "bbb:0", "reason": "bad signature",
	})
	c.EmitBroadcast(TypeBroadcastDedup, "10.0.0.1:17464", "dedup MNB", map[string]any{
		"outpoint": "aaa:0",
	})

	c.EmitPing("ping_received", "10.0.0.1:17464", "ping received", nil)
	c.EmitPing("ping_accepted", "10.0.0.1:17464", "ping accepted", nil)
	...
	c.EmitNetwork("network_mnb_received", "10.0.0.3:17464", "MNB from net", map[string]any{"outpoint": "ccc:0"})
	c.EmitNetwork("dseg_request", "10.0.0.3:17464", "DSEG request", map[string]any{"payload_size": 100})
	c.EmitNetwork("dseg_response", "10.0.0.3:17464", "DSEG response", map[string]any{"sent_count": float64(500)})
...
func TestMatchesFilter(t *testing.T) {
	...
	tests := []struct {
		...
		{"matching source", Filter{Source: "192.168.1.1:17464"}, true},
		{"non-matching source", Filter{Source: "10.0.0.1:17464"}, false},
