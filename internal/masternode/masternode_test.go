...
func TestUpdateFromBroadcast(t *testing.T) {
	t.Run("AddressUpdate", func(t *testing.T) {
		...
		oldAddr := &net.TCPAddr{IP: net.ParseIP("1.2.3.4"), Port: 17464}
		kp, _ := crypto.GenerateKeyPair()
		...
		assert.NotNil(t, manager.addressIndex["1.2.3.4:17464"])

		// Create broadcast with new address
		newAddr := &net.TCPAddr{IP: net.ParseIP("5.6.7.8"), Port: 17464}
		...
		require.NoError(t, err)
		assert.Equal(t, "5.6.7.8:17464", retrieved.Addr.String())

		// Verify address index updated
		assert.Nil(t, manager.addressIndex["1.2.3.4:17464"], "old address should be removed from index")
		assert.NotNil(t, manager.addressIndex["5.6.7.8:17464"], "new address should be in index")
	})

	t.Run("ProtocolUpgrade", func(t *testing.T) {
		manager := createTestManager(t)

		addr := &net.TCPAddr{IP: net.ParseIP("1.2.3.4"), Port: 17464}
		kp, _ := crypto.GenerateKeyPair()
...
// When SPORK_FIX_01 is disabled, higher-tier masternodes (Silver/Gold/Platinum)
...
func TestUpdateStatusWithUTXO_GetUTXOValueErrors(t *testing.T) {
	...
	t.Run("Invalid collateral amount - marked as VinSpent", func(t *testing.T) {
		...
		utxoChecker := newMockUTXOChecker()
		utxoChecker.spent[mn.OutPoint] = false
		utxoChecker.values[mn.OutPoint] = 500000 * 100000000 // 500K FIX - invalid amount
