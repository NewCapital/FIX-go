// Copyright (c) 2018-2025 The FIX developers
...
# Comment line
mn1 127.0.0.1:17464 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0
mn2 192.168.1.100:17464 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 3bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 1

# Another comment
mn3 10.0.0.1:17464 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 4bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 2 DAddr123:25
`
	...
	if entries[0].IP != "127.0.0.1:17464" {
		t.Errorf("entry 0: expected IP '127.0.0.1:17464', got '%s'", entries[0].IP)
	}
...
func TestMasternodeConfFile_GetEntry(t *testing.T) {
	...
	confContent := `mn1 127.0.0.1:17464 privkey1 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0
mn2 127.0.0.2:17464 privkey2 3bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 1
`
	confPath := filepath.Join(tmpDir, "masternode.conf")
	if err := os.WriteFile(confPath, []byte(confContent), 0644); err != nil {
		t.Fatalf("failed to write test file: %v", err)
...
func TestMasternodeEntry_GetHostPort(t *testing.T) {
	tests := []struct {
		...
		{"IPv4", "192.168.1.100:17464", "192.168.1.100", 17464},
		{"IPv4 localhost", "127.0.0.1:17464", "127.0.0.1", 17464},
		{"IPv6 full", "[2001:db8::1]:17464", "2001:db8::1", 17464},
		{"IPv6 loopback", "[::1]:17464", "::1", 17464},
		{"IPv6 all zeros", "[::]:17464", "::", 17464},
		{"IPv6 long", "[2001:0db8:85a3:0000:0000:8a2e:0370:7334]:17464", "2001:0db8:85a3:0000:0000:8a2e:0370:7334", 17464},
	}
...
mn_v4 127.0.0.1:17464 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0
mn_v6 [2001:db8::1]:17464 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 3bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 1
mn_v6loop [::1]:17464 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 4bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 2
`
	confPath := filepath.Join(tmpDir, "masternode.conf")
	if err := os.WriteFile(confPath, []byte(confContent), 0644); err != nil {
		t.Fatalf("failed to write test file: %v", err)
	...
	if entries[0].GetPort() != 17464 {
		t.Errorf("IPv4: expected port 17464, got %d", entries[0].GetPort())
	}
	...
	if entries[1].GetPort() != 17464 {
		t.Errorf("IPv6: expected port 17464, got %d", entries[1].GetPort())
	}
	...
	if entries[2].GetPort() != 17464 {
		t.Errorf("IPv6 loopback: expected port 17464, got %d", entries[2].GetPort())
	}
	...
	if e.GetPort() != 17464 {
		t.Errorf("after reload: expected port 17464, got %d", e.GetPort())
	}
}
...
func TestValidatePort(t *testing.T) {
	tests := []struct {
		...
		{"mainnet correct port", 17464, true, false},
		{"mainnet wrong port", 17465, true, true},
		{"testnet non-mainnet port", 17465, false, false},
		{"testnet mainnet port", 17464, false, true},
	}
...
func TestMasternodeConfFile_InvalidFormat(t *testing.T) {
	...
	tests := []struct {
		...
		{"too few fields", "mn1 127.0.0.1:17464 privkey"},
		{"invalid txhash length", "mn1 127.0.0.1:17464 privkey abc 0"},
		{"invalid output index", "mn1 127.0.0.1:17464 privkey 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c notanumber"},
