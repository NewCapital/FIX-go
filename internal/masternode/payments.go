...
type PaymentCalculator struct {
	...
	params     *types.ChainParams  // Network params; nil means mainnet schedule
}
...
// params selects the block-reward bootstrap boundary (mainnet vs testnet); nil defaults to mainnet.
func NewPaymentCalculator(manager *Manager, devAddress []byte, params *types.ChainParams) *PaymentCalculator {
...
// CalculateBlockReward returns the block reward for the given height,
// using the bootstrap-phase boundary appropriate for this calculator's network.
func (pc *PaymentCalculator) CalculateBlockReward(height uint32) int64 {
	return blockreward.ForParams(height, pc.params)
}

// CalculateMasternodePayment calculates the payment for a masternode
// In FIX protocol, ALL masternodes receive the SAME payment amount (80% of block reward)
...
func (pc *PaymentCalculator) FillBlockPayment(blockHeight uint32, blockHash types.Hash, isProofOfStake bool) (*BlockPaymentInfo, error) {
	...
	// Legacy: CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight + 1, blockValue, 0, fZFIXStake)
...
func (pc *PaymentCalculator) CalculateTotalMasternodeReward(blockReward int64) int64 {
	...
	// CRITICAL: Legacy FIX allocates 80% of block reward to masternodes
	// Reference: main.cpp GetMasternodePayment() returns blockValue * 0.8
	// NOT 45% like Dash - FIX has higher masternode reward share
