...
// CoinUnit is the number of satoshis in 1 FIX (base denomination unit)
...
const (
	// SporkFIXEnableMasternodeTiers controls multi-tier masternode support
	// When OFF (default): Only Bronze tier (1M FIX) is accepted
	// When ON: All 4 tiers (Bronze/Silver/Gold/Platinum) are accepted
	// Legacy C++ reference: SPORK_FIX_01_ENABLE_MASTERNODE_TIERS (20190001)
	SporkFIXEnableMasternodeTiers int32 = 20190001
)
...
func (mnb *MasternodeBroadcast) GetHash() types.Hash {
	...
	// Double SHA256 hash (Bitcoin/FIX standard)
...
// Network port constants - MUST match legacy FIX chainparams.cpp values
...
func (mn *Masternode) GetPayee() string {
	...
	// Create address from collateral public key using FIX mainnet prefix
	address := crypto.NewAddressFromPubKey(mn.PubKeyCollateral, crypto.MainNetPubKeyHashAddrID)
...
func (mn *Masternode) GetPayeeScript() []byte {
	...
	// This matches Bitcoin/FIX standard P2PKH format
...
// CRITICAL: Must use lastPing.sigTime (signed message time), NOT wall-clock receive time
// CRITICAL: multiTierEnabled controls collateral validation (legacy: SPORK_FIX_01_ENABLE_MASTERNODE_TIERS)
...
func (mn *Masternode) UpdateStatusWithUTXO(currentTime time.Time, expireTime time.Duration, utxoChecker UTXOChecker, multiTierEnabled bool) {
	...
	if utxoChecker != nil {
		...
		// Legacy: isMasternodeCollateral() checks SPORK_FIX_01_ENABLE_MASTERNODE_TIERS
		// - When spork OFF: Only base collateral (1M FIX) is valid
...
func (mnb *MasternodeBroadcast) getNewSignatureMessage() string {
	...
	// Add address string (e.g., "127.0.0.1:17464")
