// Package masternode implements the FIX masternode system with 4-tier support.
//
...
//   - Required for: Multi-tier masternode support (SPORK_FIX_01_ENABLE_MASTERNODE_TIERS)
//   - Without it: Only Bronze tier (1M FIX) is accepted, higher tiers are rejected
...
import (
	...
	"github.com/NewCapital/FIX-go/internal/consensus"
	"github.com/NewCapital/FIX-go/internal/masternode/debug"
	adjustedtime "github.com/NewCapital/FIX-go/internal/time"
...
//
// Legacy behavior:
//   - If SPORK_FIX_01_ENABLE_MASTERNODE_TIERS is OFF: Only Bronze (1M FIX) accepted
//   - If SPORK_FIX_01_ENABLE_MASTERNODE_TIERS is ON: All 4 tiers accepted
//
// Returns:
...
func (m *Manager) IsValidCollateral(amount int64) (MasternodeTier, bool) {
	// Bronze (1M FIX) is always valid regardless of spork state
	if amount == TierBronzeCollateral {
		return Bronze, true
	}

	// Higher tiers require SPORK_FIX_01_ENABLE_MASTERNODE_TIERS to be active
	m.mu.RLock()
	tiersEnabled := m.sporkManager != nil && m.sporkManager.IsActive(SporkFIXEnableMasternodeTiers)
	m.mu.RUnlock()
	...
	if !tiersEnabled {
		...
		m.logger.WithField("amount", amount).Debug("Multi-tier collateral rejected: SPORK_FIX_01 not active")
		return Bronze, false
	}
...
// This is a convenience method for checking SPORK_FIX_01_ENABLE_MASTERNODE_TIERS
func (m *Manager) IsTierSporkActive() bool {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return m.sporkManager != nil && m.sporkManager.IsActive(SporkFIXEnableMasternodeTiers)
}
...
// validateCollateralWithSpork validates collateral amount respecting SPORK_FIX_01_ENABLE_MASTERNODE_TIERS
// When spork is disabled, only Bronze (1M FIX) is accepted (single-tier mode)
...
func (m *Manager) validateCollateralWithSpork(collateral int64) (MasternodeTier, error) {
	...
	const SPORK_FIX_01_ENABLE_MASTERNODE_TIERS = int32(20190001)
	...
	if m.sporkManager != nil {
		tiersEnabled = m.sporkManager.IsActive(SPORK_FIX_01_ENABLE_MASTERNODE_TIERS)
	}
	...
	if !tiersEnabled {
		...
		return Bronze, fmt.Errorf("masternode tiers disabled: only %d FIX collateral accepted (got %d)",
...
// The tier spork (SPORK_FIX_01) only gates:
...
// isMultiTierEnabled checks if SPORK_FIX_01_ENABLE_MASTERNODE_TIERS is active
// Legacy: IsSporkActive(SPORK_FIX_01_ENABLE_MASTERNODE_TIERS) from main.cpp:115
...
func (m *Manager) isMultiTierEnabled() bool {
	...
	return sm.IsActive(SPORK_FIX_01_ENABLE_MASTERNODE_TIERS)
}

// isMultiTierEnabledLocked checks if SPORK_FIX_01_ENABLE_MASTERNODE_TIERS is active
...
func (m *Manager) isMultiTierEnabledLocked() bool {
	...
	return m.sporkManager.IsActive(SPORK_FIX_01_ENABLE_MASTERNODE_TIERS)
}
