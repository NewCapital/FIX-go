...
type CollateralValidator struct {
	...
	// Legacy C++: IsSporkActive(SPORK_FIX_01_ENABLE_MASTERNODE_TIERS) from main.cpp
...
func (cv *CollateralValidator) ValidateCollateral(outpoint types.Outpoint) (*CollateralInfo, error) {
	...
	if !ok {
		...
		if !tierEnabled {
			return nil, fmt.Errorf("%w: amount %d FIX - only Bronze (1M) accepted (SPORK_FIX_01 not active)",
				ErrCollateralInvalidTier, utxo.Value/1e8)
		}
		return nil, fmt.Errorf("%w: amount %d FIX doesn't match Bronze (1M), Silver (5M), Gold (20M), or Platinum (100M)",
			ErrCollateralInvalidTier, utxo.Value/1e8)
	}
...
// respecting the SPORK_FIX_01_ENABLE_MASTERNODE_TIERS spork status.
// When spork is OFF (or IsMultiTierEnabled is nil), only Bronze tier is valid.
// When spork is ON, all 4 tiers are valid.
// Legacy C++: isMasternodeCollateral() checks SPORK_FIX_01_ENABLE_MASTERNODE_TIERS
...
// FormatCollateralAmount formats a collateral amount in FIX (not satoshis)
func FormatCollateralAmount(satoshis int64) string {
	switch satoshis {
	case TierBronzeCollateral:
		return "1M FIX (Bronze)"
	case TierSilverCollateral:
		return "5M FIX (Silver)"
	case TierGoldCollateral:
		return "20M FIX (Gold)"
	case TierPlatinumCollateral:
		return "100M FIX (Platinum)"
	default:
		return fmt.Sprintf("%d FIX (invalid)", satoshis/CoinUnit)
	}
}
