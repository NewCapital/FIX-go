...
// handleGetAccountAddress returns the current FIX address for receiving payments to an account
...
func (s *Server) handleGetBalance(req *Request) *Response {
	...
	// Convert satoshis to FIX (divide by 1e8)
	balanceFIX := float64(balance.Confirmed) / 1e8
...
// handleSendToAddress sends FIX to an address
func (s *Server) handleSendToAddress(req *Request) *Response {
	...
	const MAX_MONEY = 21000000000.0 // 21 billion FIX
	if amount > MAX_MONEY {
		return &Response{
			JSONRPC: "2.0",
	...
	// Convert FIX amount to satoshis with precision check
	...
	if err != nil || !validation.IsValid {
		return &Response{
			JSONRPC: "2.0",
			Error:   NewInvalidParamsError("Invalid FIX address"),
			ID:      req.ID,
		}
	}
...
func (s *Server) handleListTransactions(req *Request) *Response {
	...
	// Convert amounts from satoshi to FIX
...
// handleGetRawChangeAddress returns a new FIX address for receiving change
...
func (s *Server) handleGetUnconfirmedBalance(req *Request) *Response {
	...
	// Convert satoshis to FIX (divide by 1e8)
	balanceFIX := float64(balance.Unconfirmed) / 1e8
...
func (s *Server) handleGetReceivedByAddress(req *Request) *Response {
	...
	// Convert satoshis to FIX
	amountFIX := float64(totalReceived) / 1e8
...
// handleSendFrom sends an amount from an account to a FIX address
...
// handleSendMany sends FIX to multiple addresses in one transaction
func (s *Server) handleSendMany(req *Request) *Response {
	...
	for address, amountVal := range amountsObj {
		...
		if err != nil || !validation.IsValid {
			return &Response{
				JSONRPC: "2.0",
				Error:   NewInvalidParamsError("Invalid FIX address: " + address),
				ID:      req.ID,
			}
		}
...
func (s *Server) handleListReceivedByAddress(req *Request) *Response {
	...
	// List received by address (adapter already converts to FIX)
...
func (s *Server) handleListReceivedByAccount(req *Request) *Response {
	...
	// List received by account (adapter already converts to FIX)
...
func (s *Server) handleListAddressGroupings(req *Request) *Response {
	...
	// Convert satoshis to FIX in the groupings
	...
	for i, group := range groupings {
		...
		for j, entry := range group {
			...
			for k, val := range entry {
				if k == 1 {
					// Convert amount from satoshis to FIX
...
func (s *Server) handleGetTransaction(req *Request) *Response {
	...
	// Use map[string]interface{} to output amount in FIX (not satoshis)
	...
	for vout, output := range rawTx.Outputs {
		...
		if addr != "" && s.wallet.IsOurAddress(addr) {
			details = append(details, map[string]interface{}{
				...
				"amount":   float64(output.Value) / 1e8, // Convert to FIX
				"vout":     uint32(vout),
			})
		}
	}
	...
	if len(details) == 0 && wtx.Address != "" {
		details = append(details, map[string]interface{}{
			...
			"address":  wtx.Address,
			"category": wtx.Category,
			"amount":   float64(wtx.Amount) / 1e8, // Convert to FIX
			"vout":     0,
		})
	}
...
func (s *Server) handleReserveBalance(req *Request) *Response {
	...
	if len(params) > 1 {
		if amountFloat, ok := params[1].(float64); ok {
			amount = int64(amountFloat * 1e8) // Convert FIX to satoshis
		}
	}
...
func (s *Server) handleSetStakeSplitThreshold(req *Request) *Response {
	...
	threshold := int64(thresholdFloat * 1e8) // Convert FIX to satoshis
...
// Usage: setautocombine <true|false> [target_amount_in_FIX]
func (s *Server) handleSetAutoCombine(req *Request) *Response {
	...
	// Persist to fixd.yml via ConfigManager (stores FIX, not satoshis)
	if s.configSetter != nil {
		_ = s.configSetter.Set("wallet.autoCombine", enabled)
		_ = s.configSetter.Set("wallet.autoCombineTarget", targetFIX)
...
func (s *Server) handleGetAutoCombine(req *Request) *Response {
	...
	result := map[string]interface{}{
		"enabled":  enabled,
		"target":   targetSatoshis / 100_000_000, // Convert satoshis to FIX
		"cooldown": cooldown,
	}
...
func (s *Server) handleMultiSendDisableAddress(req *Request, addrParam interface{}) *Response {
	...
	if err != nil || !validation.IsValid {
		return &Response{
			JSONRPC: "2.0",
			Error:   NewError(-5, "invalid FIX address", nil),
			ID:      req.ID,
		}
	}
