	...
	if s.wallet != nil {
		...
		if walletUnlocked {
			...
			if bal != nil && bal.Confirmed > 0 {
				mintableCoins = true
				// Check if balance exceeds reserve (assume 1 FIX reserve)
				reserve := int64(100000000) // 1 FIX in satoshis
...
func (s *Server) handleGetBlockTemplate(req *Request) *Response {
	...
	// Uses the FIX reward schedule (legacy main.cpp:1880-1928) with
	// network-aware bootstrap-phase boundary.
	blockReward := blockreward.ForParams(nextHeight, s.chainParams)
...
func (s *Server) handleEstimateFee(req *Request) *Response {
	...
	// For FIX, use a simplified fee estimation
	var feeRate float64
	...
	if s.mempool != nil {
		...
		if len(txs) == 0 {
			...
			feeRate = 0.00001 // 0.00001 FIX per kB (minimum)
		} else {
