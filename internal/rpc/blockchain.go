...
func getVersionNumber() int {
	parts := strings.Split(clilib.Version, ".")
	if len(parts) < 1 {
...
func (s *Server) handleGetInfo(req *Request) *Response {
	...
	keypoolSize := 0
	if s.wallet != nil {
		walletVersion = 61000        // FIX wallet version (matches legacy)
		bal := s.wallet.GetBalance() // Get balance
		if bal != nil {
			balance = float64(bal.Confirmed) / 1e8 // Convert satoshis to FIX
		}
	...
	protocolVersion := 70928 // FIX protocol version - must match p2p.ProtocolVersion

	result := map[string]interface{}{
		"version":         getVersionNumber(), // FIX Core version (dynamic from cli.Version)
		...
		"paytxfee":        0.0,
		"relayfee":        0.001,                      // 100,000 satoshi/KB = 0.001 FIX/KB (matches legacy minRelayTxFee)
...
// Uses FIX PoS max target (0x1e0fffff) instead of Bitcoin PoW target
...
// calculateMoneySupply retrieves total FIX in circulation at given height
...
func (s *Server) calculateMoneySupply(height uint32) float64 {
	...
	return float64(supply) / 1e8 // Convert satoshis to FIX
}
...
func (s *Server) handleGetFeeInfo(req *Request) *Response {
	...
	result := map[string]interface{}{
		...
		"minpriority": 0.0,
		"minfee":      float64(minFee) / 1e5, // FIX/kB
...
func (s *Server) handleGetRewardRates(req *Request) *Response {
	// FIX 4-tier masternode system reward distribution
	...
	result := map[string]interface{}{
		...
		"tiers": map[string]interface{}{
			"bronze": map[string]interface{}{
				"collateral":     1000000,
				"reward_percent": 10.0,
				"min_collateral": "1000000 FIX",
			},
			"silver": map[string]interface{}{
				"collateral":     5000000,
				"reward_percent": 20.0,
				"min_collateral": "5000000 FIX",
			},
			"gold": map[string]interface{}{
				"collateral":     20000000,
				"reward_percent": 30.0,
				"min_collateral": "20000000 FIX",
			},
			"platinum": map[string]interface{}{
				"collateral":     100000000,
				"reward_percent": 40.0,
				"min_collateral": "100000000 FIX",
			},
		},
	}
