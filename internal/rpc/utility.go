...
func (s *Server) handleSpork(req *Request) *Response {
	...
	if command == "show" || command == "active" {
		...
		sporkMap := map[string]int32{
			...
			"SPORK_FIX_01_ENABLE_MASTERNODE_TIERS":   20190001,
			"SPORK_FIX_02_MIN_STAKE_AMOUNT":          20190002,
		}
		...
		defaults := map[int32]int64{
			...
			20190001: 4070908800, // FIX_01_ENABLE_MASTERNODE_TIERS (OFF)
			20190002: 4070908800, // FIX_02_MIN_STAKE_AMOUNT (OFF)
		}
...
func (s *Server) handleSetTxFee(req *Request) *Response {
	...
	if len(params) != 1 {
		return &Response{
			JSONRPC: "2.0",
			Error:   NewError(-32602, "settxfee requires exactly 1 parameter (amount in FIX/kB)", nil),
			ID:      req.ID,
		}
	}
	...
	const maxTxFee = 1.0 // 1 FIX/kB maximum (reasonable upper bound)
	if amount < 0 || amount > maxTxFee {
		return &Response{
			JSONRPC: "2.0",
			Error:   NewError(-32602, fmt.Sprintf("Amount out of range (0 to %.8f FIX/kB)", maxTxFee), nil),
			ID:      req.ID,
		}
	}

	// Convert FIX/kB to satoshis/kB
	...
	s.logger.WithField("fee", fmt.Sprintf("%.8f FIX/kB", amount)).Debug("Transaction fee set")
