...
func (s *Server) handleCreateRawTransaction(req *Request) *Response {
	...
	maxReasonableOutput := int64(1000000 * 1e8) // 1 million FIX per output

	for _, output := range tx.Outputs {
		totalOutput += output.Value
		// Warn about suspiciously large individual outputs
		if output.Value > maxReasonableOutput {
			s.logger.Warnf("Output of %d satoshis (%.8f FIX) seems very large, verify amounts",
				output.Value, float64(output.Value)/1e8)
		}
	}

	// Also warn if total outputs seem excessive (more than 21 million FIX worth of satoshis)
