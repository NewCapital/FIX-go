...
// If WindowTitle is empty, returns "FIX Core" as default.
...
func GetDefaultGUIDataDir() string {
	...
	case "windows":
		...
		if appData != "" {
			return filepath.Join(appData, "FIX")
		}
		return filepath.Join(home, "AppData", "Roaming", "FIX")
	case "darwin":
		return filepath.Join(home, "Library", "Application Support", "FIX")
	default: // Linux and others
		return filepath.Join(home, ".fix")
	}
}
...
func PrintGUIHelp() {
	fmt.Println("FIX Core Wallet - GUI Application")
	fmt.Println()
	fmt.Println("Usage: fix-gui [options]")
	fmt.Println()
	fmt.Println("Options:")
	fmt.Println()
	...
	fmt.Printf("  Windows:  %%APPDATA%%\\FIX\n")
	fmt.Printf("  macOS:    ~/Library/Application Support/FIX\n")
	fmt.Printf("  Linux:    ~/.fix\n")
}

// PrintGUIVersion prints version information to stdout
func PrintGUIVersion() {
	fmt.Printf("FIX Core Wallet (GUI) version %s\n", GUIVersion)
