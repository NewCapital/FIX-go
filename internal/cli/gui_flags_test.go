...
func TestParseGUIArgs_DataDir(t *testing.T) {
	tempDir := t.TempDir()
	testPath := tempDir + "/fix-data"
	args := []string{"fix-gui", "-datadir=" + testPath}
	config, shouldExit, err := ParseGUIArgs(args)
...
func TestParseGUIArgs_TestnetAndRegtest_Error(t *testing.T) {
	args := []string{"fix-gui", "-testnet", "-regtest"}
...
func TestParseGUIArgs_SplashExplicitTrue(t *testing.T) {
	args := []string{"fix-gui", "-splash=true"}
	config, shouldExit, err := ParseGUIArgs(args)
...
func TestParseGUIArgs_SplashExplicitFalse(t *testing.T) {
	args := []string{"fix-gui", "-splash=false"}
	config, shouldExit, err := ParseGUIArgs(args)
...
func TestParseGUIArgs_ChooseDataDir(t *testing.T) {
	args := []string{"fix-gui", "-choosedatadir"}
	config, shouldExit, err := ParseGUIArgs(args)
...
func TestParseGUIArgs_WindowTitle(t *testing.T) {
	args := []string{"fix-gui", "-windowtitle=MyWallet"}
	config, shouldExit, err := ParseGUIArgs(args)
...
func TestParseGUIArgs_CombinedFlags(t *testing.T) {
	tempDir := t.TempDir()
	testPath := tempDir + "/fix-data"
	args := []string{"fix-gui", "-testnet", "-min", "-nosplash", "-datadir=" + testPath, "-lang=uk"}
	config, shouldExit, err := ParseGUIArgs(args)
...
func TestParseGUIArgs_GNUStyle(t *testing.T) {
	args := []string{"fix-gui", "--testnet", "--min"}
	config, shouldExit, err := ParseGUIArgs(args)
...
func TestExpandPath_AcceptsValidPaths(t *testing.T) {
	testCases := []struct {
		...
		{"absolute unix path", "/home/user/fix-data"},
		{"home expansion", "~/fix-data"},
		{"relative path", "fix-data"},
	}
...
func TestGetDefaultGUIDataDir(t *testing.T) {
	...
	switch runtime.GOOS {
	case "windows":
		if !containsIgnoreCase(dataDir, "FIX") {
			t.Errorf("Windows dataDir should contain FIX: %q", dataDir)
		}
	case "darwin":
		if !containsIgnoreCase(dataDir, "FIX") {
			t.Errorf("macOS dataDir should contain FIX: %q", dataDir)
		}
	default:
		if !containsIgnoreCase(dataDir, ".fix") {
			t.Errorf("Linux dataDir should contain .fix: %q", dataDir)
		}
	}
}
