	...
	app := &cli.App{
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{
				...
				EnvVars: []string{"FIX_NETWORK"},
			},
			&cli.StringFlag{
				Name:    "log-level",
				Value:   "error",
				Usage:   "Logging level (trace, debug, info, warn, error)",
				EnvVars: []string{"FIX_LOG_LEVEL"},
			},
			&cli.BoolFlag{
				Name:    "log-json",
				Value:   false,
				Usage:   "Output logs in JSON format",
				EnvVars: []string{"FIX_LOG_JSON"},
			},
			&cli.StringFlag{
				Name:    "log-file",
				Value:   "",
				Usage:   "Write logs to file (in addition to stdout)",
				EnvVars: []string{"FIX_LOG_FILE"},
			},
		},
		Authors: []*cli.Author{
			{
				Name:  "FIX Development Team",
				Email: "noreply@fix.network",
			},
		},
		Copyright: "Copyright © 2025 FIX Development Team",
	}
...
func getDefaultConfigPath(appName string) string {
	...
	return filepath.Join(homeDir, ".fix", appName+".yml")
}

// getDefaultDataDir returns the default data directory
// All files (blockchain.db, wallet.dat, txcache.dat, etc.) are stored directly in ~/.fix
func getDefaultDataDir(appName string) string {
	homeDir, _ := os.UserHomeDir()
	return filepath.Join(homeDir, ".fix")
}
...
func SetupLogging(c *cli.Context) error {
	...
	// Log file output is deferred to after config loading (startup_improved.go / fix-gui app.go)
	// so that logging.output from fixd.yml is respected. The --log-file CLI flag is also
...
// This allows flags specified before subcommand (e.g., "fixd -d /path start")
...
// GetStringFromLineage searches for a string flag value through context lineage.
// Exported wrapper for use by cmd packages (fix-cli, fixd).
func GetStringFromLineage(c *cli.Context, name string) string {
...
// GetFIXBaseDir returns the base .fix directory (~/.fix)
// This is where config files live (fixd.yml, fix.conf)
func GetFIXBaseDir() string {
	homeDir, _ := os.UserHomeDir()
	return filepath.Join(homeDir, ".fix")
}
...
// Only fixd.yml is supported. If a legacy fix.conf exists, a warning is logged.
// Returns empty string if no config file found.
func DiscoverConfigFile() string {
	baseDir := GetFIXBaseDir()

	ymlPath := filepath.Join(baseDir, "fixd.yml")
	if _, err := os.Stat(ymlPath); err == nil {
		return ymlPath
	}

	// Warn about legacy fix.conf if present
	confPath := filepath.Join(baseDir, "fix.conf")
	if _, err := os.Stat(confPath); err == nil {
		logrus.WithField("path", confPath).Warn(
			"Found legacy fix.conf but this format is no longer supported. " +
				"Please migrate to fixd.yml (YAML format). " +
				"Run fixd once to auto-generate a default fixd.yml.")
	}
