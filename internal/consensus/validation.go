...
import (
	...
	"github.com/NewCapital/FIX-go/pkg/blockreward"
	"github.com/NewCapital/FIX-go/pkg/crypto"
	"github.com/NewCapital/FIX-go/pkg/script"
...
func (bv *BlockValidator) ValidateBlock(ctx *ValidationContext) error {
	...
	// Legacy: main.cpp:3968-3972 - validates vout[1].Value >= MinStakeAmount with spork+height gate
...
func (bv *BlockValidator) ValidateBlockHeader(header, prevHeader *types.BlockHeader, height uint32) error {
	...
	// Legacy FIX allows timestamps to be slightly out of order due to clock drift
...
func (bv *BlockValidator) ValidateTarget(block *types.Block, height uint32) error {
	...
	// Validate target is within network limits. PoW blocks (height <= LastPOWBlock)
	// are bounded by the PoW limit; PoS blocks are bounded by the tighter PoS
	// limit (legacy bnTargetLimit = ~uint256(0) >> 24 = MaxTargetPoS).
	// Without this separation the emergency-window tolerance paths below could
	// accept a PoS block whose target sits between MaxTargetPoS (0x1e00ffff)
	// and the PoW limit (0x1e0fffff), opening a ~16x-easier Go-only fork vector
	// since the kernel check reuses header.Bits as the staking target.
	target := GetTargetFromBits(header.Bits)
	maxTarget := bv.params.PowLimitBig
	if height > bv.params.LastPOWBlock {
		maxTarget = MaxTargetPoS
	}
	...
	// Legacy emergency-difficulty validation tolerance. For PoS blocks at
	// pindexPrev->nHeight >= 1487457 the legacy C++ patch introduces a
	// GetNextWorkRequired branch that caps the target at the PoS limit after a
	// gap > 10 target spacings (legacy/src/pow.cpp:55-75). It also adds a
	// CheckWork tolerance that accepts blocks with target >= required
	// (legacy/src/main.cpp:4078-4097 — "easier than required"). The real FIX
	// mainnet chain contains BOTH pre-patch blocks (plain ppcoin bits) and
	// post-patch blocks (emergency-capped bits) at this height range because
	// the patch was not uniformly deployed before new blocks were mined. To
	// sync across that boundary we accept either form:
	//   1. exact emergency match (default expectedBits)
	//   2. "easier than emergency" — legacy CheckWork tolerance
	//   3. exact ppcoin match — unpatched-miner block
	if block.IsProofOfStake() && height > 0 && uint32(height-1) >= EmergencyDifficultyStartPrevHeight {
		nTargetSpacing := int64(TargetSpacingSeconds)
		if height > 0 && height-1 < BlockHeightTargetSpacingChange {
			nTargetSpacing = int64(TargetSpacingSecondsOld)
		}
		emergencyThreshold := nTargetSpacing * EmergencyDifficultyDelayFactor

		var prevTimestamp int64 = -1
		if bv.pos.blockchain != nil {
			if prevBlock, err := bv.pos.blockchain.GetBlock(header.PrevBlockHash); err == nil &&
				prevBlock != nil && prevBlock.Header != nil {
				prevTimestamp = int64(prevBlock.Header.Timestamp)
			}
		}

		if prevTimestamp > 0 {
			gap := int64(header.Timestamp) - prevTimestamp
			if gap > emergencyThreshold {
				// Path 2: legacy CheckWork easier-than-required tolerance.
				actualTarget := GetTargetFromBits(header.Bits)
				expectedTarget := GetTargetFromBits(expectedBits)
				if actualTarget.Cmp(expectedTarget) >= 0 {
					bv.pos.logger.WithFields(logrus.Fields{
						"height":        height,
						"gap_sec":       gap,
						"actual_bits":   fmt.Sprintf("0x%08x", header.Bits),
						"expected_bits": fmt.Sprintf("0x%08x", expectedBits),
					}).Debug("Accepting emergency-difficulty block (legacy CheckWork tolerance)")
					return nil
				}

				// Path 3: unpatched-miner ppcoin block. Recompute the expected
				// bits with the emergency branch disabled; if they match the
				// block's bits exactly, the block was produced by a legacy
				// node without the emergency patch — still canonical.
				ppcoinBits, err := bv.pos.CalculateNextWorkRequiredNoEmergency(header, height)
				if err == nil && header.Bits == ppcoinBits {
					bv.pos.logger.WithFields(logrus.Fields{
						"height":        height,
						"gap_sec":       gap,
						"actual_bits":   fmt.Sprintf("0x%08x", header.Bits),
						"expected_bits": fmt.Sprintf("0x%08x", expectedBits),
						"ppcoin_bits":   fmt.Sprintf("0x%08x", ppcoinBits),
					}).Debug("Accepting unpatched-miner ppcoin block during emergency window")
					return nil
				}
			}
		}
...
// CalculateBlockReward calculates the block reward for the given height,
// using the bootstrap-phase boundary appropriate for this validator's network.
func (bv *BlockValidator) CalculateBlockReward(height uint32) int64 {
	return blockreward.ForParams(height, bv.params)
}

// GetBlockValue returns the mainnet block reward for the given height.
// Backwards-compatible standalone wrapper around blockreward.Mainnet.
// Callers with ChainParams in hand should use blockreward.ForParams directly.
func GetBlockValue(height uint32) int64 {
	return blockreward.Mainnet(height)
}
...
// Legacy: main.cpp:3968-3972 - CheckBlock min stake validation
...
func (bv *BlockValidator) validateMinStakeOutput(block *types.Block, height uint32) error {
	...
	const SPORK_FIX_02_MIN_STAKE_AMOUNT = int32(20190002)

	// Height thresholds from legacy main.cpp:3969
	// Mainnet: 223000, Testnet: 192500
	// Corroborated by legacy chainparams.cpp:70 checkpoint at height 223000
	// labelled "min stake enforcement" (mirrored in internal/blockchain/checkpoints.go:58).
	const minStakeHeightMainnet = uint32(223000)
	const minStakeHeightTestnet = uint32(192500)

	// LEGACY COMPLIANCE: Legacy C++ gates this rule on
	// IsSporkActive(SPORK_FIX_02_MIN_STAKE_AMOUNT). On mainnet the spork was
	...
	if bv.params.Name != "mainnet" {
		...
		if bv.sporkManager != nil {
			sporkActive = bv.sporkManager.IsActive(SPORK_FIX_02_MIN_STAKE_AMOUNT)
		}
		if !sporkActive {
			return nil
		}
	...
	// Legacy: nHeight >= (Params().NetworkID() == CBaseChainParams::MAIN ? 223000 : 192500)
