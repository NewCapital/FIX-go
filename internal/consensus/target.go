...
// Target calculation constants for FIX PoS
const (
	...
	TargetSpacingSeconds           = 2 * 60  // 2 minutes per block (after block 2000)
	TargetSpacingSecondsOld        = 60      // 1 minute per block (before block 2000)
	DifficultyAdjustmentInterval   = 1       // Adjust every block for PoS
	BlockHeightTargetSpacingChange = 2000    // Legacy pow.cpp uses BlockLastSolved->nHeight < 2000
	...
	EmergencyDifficultyStartPrevHeight = 1487457
	EmergencyDifficultyDelayFactor     = 10
	EmergencyDifficultyMaxShifts       = 10
...
var (
	...
	// Legacy: bnTargetLimit = ~uint256(0) >> 24 (pow.cpp)
...
func (tc *TargetCalculator) CalculateNextTarget(prevHeader, currentHeader *types.BlockHeader, currentHeight uint32) (*big.Int, error) {
	return tc.calculateNextTargetImpl(prevHeader, currentHeader, currentHeight, false)
}

// CalculateNextTargetNoEmergency calculates the next target as if the legacy
// emergency difficulty drop patch were not deployed. Used by the validator to
// accept historical PoS blocks mined by unpatched nodes during the emergency
// activation window — the FIX mainnet chain contains blocks produced by both
// patched (emergency-aware) and unpatched (plain ppcoin) miners, so strict
// single-mode validation cannot sync past that window.
func (tc *TargetCalculator) CalculateNextTargetNoEmergency(prevHeader, currentHeader *types.BlockHeader, currentHeight uint32) (*big.Int, error) {
	return tc.calculateNextTargetImpl(prevHeader, currentHeader, currentHeight, true)
}

func (tc *TargetCalculator) calculateNextTargetImpl(prevHeader, currentHeader *types.BlockHeader, currentHeight uint32, skipEmergency bool) (*big.Int, error) {
	...
	// Get target spacing based on block height. Legacy pow.cpp:44 gates on
	// `BlockLastSolved->nHeight < 2000`, i.e. the PREVIOUS block's height —
	// which is currentHeight-1. Block at currentHeight=2000 still uses the
	// old 60s spacing (prev = 1999 < 2000); the switch takes effect at 2001.
	nTargetSpacing := int64(TargetSpacingSeconds)
	if currentHeight > 0 && currentHeight-1 < BlockHeightTargetSpacingChange {
	...
	// Legacy emergency difficulty drop, activated for block 1487458 onward.
	// If the candidate block is more than 10 target spacings after the previous
	// block, increase the target 4x per delay window, capped at the PoS limit.
	if !skipEmergency && prevHeader.Timestamp > 0 && currentHeight > 0 && currentHeight-1 >= EmergencyDifficultyStartPrevHeight {
		nTimeSinceLastBlock := int64(currentHeader.Timestamp) - int64(prevHeader.Timestamp)
		if nTimeSinceLastBlock > nTargetSpacing*EmergencyDifficultyDelayFactor {
			bnEmergency := GetTargetFromBits(prevHeader.Bits)
			nMultiplier := nTimeSinceLastBlock / (nTargetSpacing * EmergencyDifficultyDelayFactor)
			for i := int64(0); i < nMultiplier && i < EmergencyDifficultyMaxShifts; i++ {
				bnEmergency.Lsh(bnEmergency, 2)
			}
			if bnEmergency.Cmp(MaxTargetPoS) > 0 {
				bnEmergency.Set(MaxTargetPoS)
			}
			return bnEmergency, nil
		}
	}
