...
//
// LEGACY CONSENSUS: This must emulate the exact semantics of legacy
// `stakeTargetHit` at legacy/src/kernel.cpp:286-293:
//
//	uint256 bnCoinDayWeight = uint256(nValueIn) / 100;
//	return hashProofOfStake < (bnCoinDayWeight * bnTargetPerCoinDay);
//
// Legacy `uint256` multiplication at legacy/src/uint256.cpp:76-89 is a
// fixed-width 256-bit operation whose inner loop `for (int i = 0; i + j <
// WIDTH; i++)` silently drops contributions at word indices >= WIDTH, i.e.
// any bits beyond 2^256 are discarded. The result is `(a * b) mod 2^256`.
//
// Go's math/big.Int is arbitrary precision and does NOT overflow. Without
// the Mod below, Go computes the exact mathematical product while legacy
// sees a wrapped (much smaller) value, causing Go to accept blocks that
// legacy rejects (see task h-fix-stake-kernel-uint256-overflow and the
// research task ?-research-stake-target-comparison-divergence).
func StakeTargetHit(kernelHash types.Hash, stakeWeight int64, target *big.Int) bool {
	...
	// EffectiveTarget = (Target * StakeWeight) mod 2^256
	// The Mod is what makes this consensus-equivalent to legacy uint256.
	effectiveTarget := new(big.Int).Mul(target, big.NewInt(stakeWeight))
	effectiveTarget.Mod(effectiveTarget, uint256Modulus)

	// Check if kernel hash is less than effective target
	return kernelBig.Cmp(effectiveTarget) < 0
}

// uint256Modulus is 2^256, used to emulate fixed-width 256-bit wrap-around
// semantics of legacy C++ `uint256` arithmetic (legacy/src/uint256.cpp:76-89).
