...
// ProofOfStake implements the FIX PoS consensus mechanism
...
// PoS algorithm constants (FIX specific)
...
// SPORK_16_ZEROCOIN_MAINTENANCE_MODE is permanently enabled on FIX mainnet,
...
// CalculateNextWorkRequiredNoEmergency returns the next difficulty bits computed
// WITHOUT the legacy emergency difficulty drop. Used by the block validator to
// accept PoS blocks mined by unpatched legacy nodes during the emergency window:
// those blocks carry plain ppcoin bits even though the patched algorithm would
// have produced a capped (emergency) target. Outside the emergency window this
// returns the same result as CalculateNextWorkRequired.
func (pos *ProofOfStake) CalculateNextWorkRequiredNoEmergency(header *types.BlockHeader, height uint32) (uint32, error) {
	if header == nil {
		return 0, errors.New("header is nil")
	}
	...
	calculator := NewTargetCalculator(pos.params, pos.targetCache, pos.storage, pos.blockchain)
	nextTarget, err := calculator.CalculateNextTargetNoEmergency(prevBlock.Header, header, height)
	if err != nil {
		return 0, err
	}
...
func (pos *ProofOfStake) getStakeInput(input *types.TxInput, blockHeight uint32) (*StakeInput, error) {
	...
	// in validation.go with proper spork + height gating per main.cpp:3968-3972.
...
func (pos *ProofOfStake) StartStaking() error {
	...
	// Legacy: wallet.cpp:3337 - FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZFIX())
