...
func TestCalculateStakeTarget(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "large weight",
			weight:         1000000000, // 10 FIX equivalent
			targetSpacing:  params.TargetSpacing,
...
func TestValidateStakeAmount(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "valid amount",
			amount:   1000000000, // 10 FIX
			minStake: 500000000,  // 5 FIX
			expected: true,
		},
		{
			name:     "exact minimum",
...
func TestCalculateStakeReward(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "large stake",
			stakeAmount:   10000000000, // 100 FIX
...
func TestSelectBestStakeInputs(t *testing.T) {
	...
	inputs := []*StakeInput{
		{Value: 1000000000, BlockTime: 1640995200, Age: 86400},           // 10 FIX, 1 day
		{Value: 5000000000, BlockTime: 1640995200 - 172800, Age: 172800}, // 50 FIX, 2 days
		{Value: 2000000000, BlockTime: 1640995200 - 86400, Age: 86400},   // 20 FIX, 1 day
	}
...
func TestCalculateNetworkStakeWeight(t *testing.T) {
	...
	// Create test blocks
	blocks := []*types.Block{
		createTestBlockWithValue(t, 0, 1000000000), // 10 FIX
		createTestBlockWithValue(t, 1, 2000000000), // 20 FIX
		createTestBlockWithValue(t, 2, 5000000000), // 50 FIX
	}
