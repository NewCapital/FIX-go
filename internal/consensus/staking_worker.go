...
func (sw *StakingWorker) tryStake() {
	...
	// Stake with plain ppcoin difficulty (no emergency cap). Live FIX mainnet
	// peers run unpatched legacy without the emergency-difficulty branch, so
	// blocks carrying the emergency-capped value (0x1e00ffff after a long gap)
	// are rejected by the majority with "incorrect proof of work". Emitting
	// ppcoin bits matches what unpatched legacy `GetNextWorkRequired` returns
	// and what historical chain blocks (e.g. 1487458..) actually carry. Our
	// validator's dual-acceptance in ValidateTarget still accepts both forms
	// for inbound blocks, so we remain lenient on receive and strict on send.
	nextBits, err := sw.consensus.CalculateNextWorkRequiredNoEmergency(newBlockHeader, newBlockHeight)
	if err != nil {
...
// zFIX/zPoS disabled via SPORK_16_ZEROCOIN_MAINTENANCE_MODE on mainnet.
// See legacy/src/wallet.cpp:2426 for zFIX stake selection (disabled).
