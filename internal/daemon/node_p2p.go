...
import (
	...
	"github.com/NewCapital/FIX-go/internal/mempool"
...
// This replaces:
//   - cmd/fixd/startup_improved.go:startP2PAndSync()
//   - cmd/fix-gui/app.go:StartP2P()
//
...
// This replaces:
//   - cmd/fixd/startup_improved.go:startRPCServer()
//   - cmd/fix-gui/app.go:StartRPCServer()
func (n *Node) InitRPC(cfg RPCConfig) error {
	...
	// so that rpc.host/rpc.port from fixd.yml are respected.
	...
	if cfg.FullConfig != nil {
		...
		// Double-gate: AllowPlaintextPublic requires BOTH fixd.yml setting AND
	...
	// Load credentials with priority: explicit > config file > fix.conf > cookie auth
	...
	if username != "" && password != "" {
		...
		var err error
		username, password, err = parseFixConf(n.Config.DataDir)
		if err != nil {
			if !os.IsNotExist(err) {
				n.logger.WithError(err).Error("fix.conf exists but could not be parsed")
			}
...
// parseFixConf reads rpcuser and rpcpassword from legacy fix.conf file.
func parseFixConf(dataDir string) (username, password string, err error) {
	confPath := filepath.Join(dataDir, "fix.conf")
