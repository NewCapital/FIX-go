...
type WalletConfig struct {
	...
	// Config file settings (optional, from fixd.yml)
...
// This replaces:
//   - cmd/fixd/startup_improved.go:startWalletRescan()
//   - cmd/fix-gui/app.go:initializeWallet()
func (n *Node) InitWallet(cfg WalletConfig) error {
	...
	// Wire autocombine from config if available (config stores FIX, wallet uses satoshis)
	if n.ConfigManager != nil {
		acEnabled := n.ConfigManager.GetBool("wallet.autoCombine")
		acTargetFIX := n.ConfigManager.GetInt64("wallet.autoCombineTarget")
		acTargetSatoshis := acTargetFIX * 100_000_000
