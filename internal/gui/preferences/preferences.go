...
func getPreferencesPath() string {
	...
	switch runtime.GOOS {
	case "windows":
		return filepath.Join(homeDir, "AppData", "Local", "FIX-Wallet", "preferences.json")
	case "darwin":
		return filepath.Join(homeDir, "Library", "Preferences", "com.fix.wallet", "preferences.json")
	default: // Linux and others
		configDir := os.Getenv("XDG_CONFIG_HOME")
		if configDir == "" {
			configDir = filepath.Join(homeDir, ".config")
		}
		return filepath.Join(configDir, "fix-wallet", "preferences.json")
	}
}
