...
func (tn *TransactionNotifier) handleNewTransaction(txEvent core.TransactionReceivedEvent) {
	...
	if isIncoming {
		...
		message = fmt.Sprintf("Received %.8f FIX", txEvent.Amount)
		notifType = NotifTypeTransactionIn
		severity = LevelSuccess
	} else {
		title = "Outgoing Transaction"
		message = fmt.Sprintf("Sent %.8f FIX", -txEvent.Amount)
...
func (tn *TransactionNotifier) handleStakingReward(stakingEvent core.StakeRewardEvent) {
	...
	notif := Notification{
		...
		Message:  fmt.Sprintf("Earned %.8f FIX from staking", stakingEvent.Amount),
		Severity: LevelSuccess,
		Data: map[string]interface{}{
...
func (tn *TransactionNotifier) handleMasternodePayment(mnEvent core.MasternodePaymentReceivedEvent) {
	...
	notif := Notification{
		...
		Message:  fmt.Sprintf("Received %.8f FIX from masternode %s", mnEvent.Amount, mnEvent.Alias),
		Severity: LevelSuccess,
		Data: map[string]interface{}{
			"alias":  mnEvent.Alias,
			"txid":   mnEvent.TxID,
