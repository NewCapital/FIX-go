...
// generateAddress generates a realistic FIX address with proper Base58Check encoding
func (m *MockCoreClient) generateAddress() string {
	// FIX mainnet P2PKH addresses use version byte 30 (0x1E) which gives prefix 'D'
...
func (m *MockCoreClient) generateInitialTransactions(count int) {
	...
	for i := 0; i < count; i++ {
		...
		case core.TxTypeStakeMint:
			amount = 38.0 + m.rng.Float64()*2.0 // Stake rewards around 38-40 FIX
		case core.TxTypeMNReward:
			amount = 11.4 + m.rng.Float64()*0.6 // MN rewards around 11.4-12 FIX
		...
		if amount < 0 { // Send transactions have fees
			fee = 0.0001 + m.rng.Float64()*0.001 // 0.0001 to 0.0011 FIX
		}
...
func (m *MockCoreClient) generateInitialUTXOs(count int) {
	...
	for i := 0; i < count; i++ {
		...
		case 8: // 10% staking rewards sized (~38-40 FIX)
			amount = 38.0 + m.rng.Float64()*2.0
		case 9: // 10% masternode rewards sized (~11.4-12 FIX)
