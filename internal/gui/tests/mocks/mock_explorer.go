...
func (m *MockCoreClient) SearchExplorer(query string) (core.SearchResult, error) {
	...
	// Check if it's a valid FIX address
	if isValidFIXAddress(query) {
		m.mu.RUnlock()
...
// isValidFIXAddress checks if a string looks like a valid FIX address
func isValidFIXAddress(s string) bool {
	// FIX addresses start with 'D' and are 34 characters
	if len(s) != 34 {
		return false
	}
	if s[0] != 'D' {
