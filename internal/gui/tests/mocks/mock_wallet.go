...
// Transaction constants for FIX network
const (
	// DustThreshold is the minimum amount for a valid FIX transaction
	// This should match the dust threshold in FIX Core (usually DUST_HARD_LIMIT)
	// TODO: Verify this value against actual FIX Core implementation
	DustThreshold = 0.001
...
func (m *MockCoreClient) SendToAddress(address string, amount float64, comment string) (string, error) {
	...
	if amount < DustThreshold {
		return "", fmt.Errorf("amount below dust threshold: minimum %.8f FIX", DustThreshold)
	}
...
func (m *MockCoreClient) validateAddressLocked(address string) (core.AddressValidation, error) {
	...
	if len(address) == 34 {
		firstChar := address[0]
		// Check for valid FIX address prefix (mainnet only for now)
...
func (m *MockCoreClient) simulateNormalConfirmation(txid string) {
	...
	// Normal FIX block time is approximately 2 minutes
...
func (m *MockCoreClient) ExportFilteredTransactionsCSV(filter core.TransactionFilter) (string, error) {
	...
	csv := "\"Confirmed\",\"Date\",\"Type\",\"Label\",\"Address\",\"Amount (FIX)\",\"ID\"\n"
	for _, txid := range m.txList {
		if tx, ok := m.transactions[txid]; ok {
			confirmed := "false"
...
// Returns the fee rate in FIX per KB for confirmation within targetBlocks
...
func (m *MockCoreClient) CalculateFee(numInputs, numOutputs int, targetBlocks int) (float64, error) {
	...
	// Round up to nearest satoshi (0.00000001 FIX)
...
// Returns the fee in FIX, input count, and transaction size for UI display.
