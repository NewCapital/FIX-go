...
// GetDefaultDataDirectory returns the default FIX data directory
func (s *Service) GetDefaultDataDirectory() string {
	...
	case "windows":
		return filepath.Join(homeDir, "AppData", "Roaming", "FIX")
	case "darwin":
		return filepath.Join(homeDir, "Library", "Application Support", "FIX")
...
func (s *Service) SelectDataDirectory(ctx context.Context) (string, error) {
	options := wailsruntime.OpenDialogOptions{
		Title: "Select FIX Data Directory",
	}
...
func (s *Service) CheckDiskSpace(directory string) (*DiskSpaceInfo, error) {
	...
	if stat.IsDir() {
		testFile := filepath.Join(dir, ".fix_test")
...
//
//	~/.fix/               (base directory = "directory" parameter)
//	├── fixd.yml          # config file (YAML, recommended)
...
func (s *Service) InitializeDataDirectory(directory string) error {
	...
	// fixd.yml creation is handled by ConfigManager.LoadOrCreate()
...
func (s *Service) GetConfiguration() *FIXConfig {
	return s.config
}
