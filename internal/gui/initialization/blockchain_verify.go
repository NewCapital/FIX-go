...
// GenesisBlock represents the FIX genesis block parameters
...
// FIX mainnet genesis block parameters
var FIXGenesis = GenesisBlock{
...
// FIX testnet genesis block parameters
var FIXTestnetGenesis = GenesisBlock{
...
func verifyGenesisBlock(info *BlockchainInfo, genesis GenesisBlock) error {
	...
	// Read the magic bytes (FIX uses the same as Bitcoin)
	magic := make([]byte, 4)
	if _, err := file.Read(magic); err != nil {
		return fmt.Errorf("failed to read magic bytes: %w", err)
	}

	// FIX mainnet magic: 0xf9beb4d9
	...
	// Reverse for display (FIX uses little-endian)
