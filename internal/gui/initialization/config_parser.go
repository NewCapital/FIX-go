...
// FIXConfig represents the parsed fix.conf configuration
...
// ParseConfigFile parses the fix.conf configuration file
func ParseConfigFile(configPath string) (*FIXConfig, error) {
	config := &FIXConfig{
		// Set defaults
		Network:         "mainnet",
		RPCPort:         17464,
	...
	for scanner.Scan() {
		...
		case "testnet":
			...
			if config.Testnet {
				...
				config.RPCPort = 17464 // Testnet port
			}
		case "regtest":
			config.Regtest = parseBool(value)
			if config.Regtest {
				config.Network = "regtest"
				config.RPCPort = 17464 // Regtest port
			}
...
func ValidateConfig(config *FIXConfig) error {
	...
	if config.Masternode {
		...
		// Validate private key format (should be 51 characters for FIX)
...
// ParseYAMLConfigFile parses a fixd.yml YAML configuration file into FIXConfig.
// Reuses internal/config.LoadConfig() for YAML parsing and maps fields to FIXConfig.
func ParseYAMLConfigFile(configPath string) (*FIXConfig, error) {
...
// ConfigFromYAML maps a config.Config (daemon YAML config) to FIXConfig (GUI config).
func ConfigFromYAML(cfg *config.Config) *FIXConfig {
	network := "mainnet"
