...
// FIX daemon settings delegate to ConfigManager (set via SetConfigManager after daemon starts).
...
// Call this after daemon.NewNode() returns so FIX config methods have a live source.
...
// Only masternode entries are imported; FIX daemon settings must be changed via ConfigManager.
...
// FIX daemon settings are sourced from ConfigManager when available.
func (s *Service) GetFullConfiguration() *CombinedConfig {
	masternodes := s.manager.GetMasternodes()
	network := s.GetNetwork()

	var fix *initialization.FIXConfig
	if cm := s.cm.Load(); cm != nil {
		fix = initialization.ConfigFromYAML(cm.Snapshot())
	}
