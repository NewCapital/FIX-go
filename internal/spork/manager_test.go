// Copyright (c) 2025 The FIX Core developers
...
func TestIsActive(t *testing.T) {
	...
	// SporkFIXEnableMasternodeTiers default is 4070908800 (year 2099), which is in future
	if manager.IsActive(SporkFIXEnableMasternodeTiers) {
		t.Error("SporkFIXEnableMasternodeTiers should not be active (default timestamp is in future)")
	}
...
func TestGetSporkName(t *testing.T) {
	tests := []struct {
		...
		{SporkFIXEnableMasternodeTiers, "SPORK_FIX_01_ENABLE_MASTERNODE_TIERS"},
		{SporkFIXMinStakeAmount, "SPORK_FIX_02_MIN_STAKE_AMOUNT"},
		{99999, "Unknown"},
	}
...
func TestGetSporkID(t *testing.T) {
	tests := []struct {
		...
		{"SPORK_FIX_01_ENABLE_MASTERNODE_TIERS", SporkFIXEnableMasternodeTiers},
		{"SPORK_FIX_02_MIN_STAKE_AMOUNT", SporkFIXMinStakeAmount},
		{"UNKNOWN_SPORK", -1},
	}
