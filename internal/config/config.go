...
// Config represents the complete FIX node configuration
type Config struct {
...
type NetworkConfig struct {
	...
	Port             int      // FIX mainnet P2P port (legacy: nDefaultPort)
...
type RPCConfig struct {
	Enabled              bool
	Port                 int // FIX RPC port (not Bitcoin 8332)
	Host                 string
	Username             string
	Password             string
	MaxClients           int
...
// RPCClientTLSConfig contains TLS settings for the RPC client (fix-cli)
...
type StakingConfig struct {
	...
	// Value is in satoshis (1 FIX = 100,000,000 satoshis)
	...
	// Value is in FIX (whole coins, e.g. 20000)
	// Legacy: was in wallet.dat, now in fixd.yml
...
type WalletConfig struct {
	...
	MaxTxFee        int64 // Maximum total fee allowed in satoshis, 1 FIX (legacy: -maxtxfee)
	...
	AutoCombineTarget   int64 // Target amount in FIX (whole coins, e.g. 200000)
