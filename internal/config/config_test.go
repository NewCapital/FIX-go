...
func TestDefaultConfig(t *testing.T) {
	...
	if config.Network.Port != 17464 {
		t.Errorf("Expected default port 17464, got %d", config.Network.Port)
	}
	...
	// Test RPC settings
	if config.RPC.Port != 17465 {
		t.Errorf("Expected RPC port 17465, got %d", config.RPC.Port)
	}
}
...
func TestGetDefaultPorts(t *testing.T) {
	tests := []struct {
		...
		{"testnet", 5447, 17467},
		{"regtest", 5467, 17467},
		{"invalid", 17464, 17465}, // Falls back to mainnet
	}
