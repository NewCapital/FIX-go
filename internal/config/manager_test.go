...
func TestGetSetInt(t *testing.T) {
	...
	if cm.GetInt("network.port") != 17464 {
		t.Errorf("expected default port 17464, got %d", cm.GetInt("network.port"))
	}
...
func TestLoadOrCreate_NewFile(t *testing.T) {
	...
	if _, err := os.Stat(yamlPath); os.IsNotExist(err) {
		t.Error("expected fixd.yml to be created")
	}
	...
	content := string(data)
	if !strings.Contains(content, "# FIX Core Configuration") || !strings.Contains(content, "staking:") || !strings.Contains(content, "network:") || !strings.Contains(content, "rpc:") {
...
`
	...
	if cm.GetInt("rpc.port") != 17465 {
		t.Errorf("expected rpc.port default 17465, got %d", cm.GetInt("rpc.port"))
	}
}
...
func TestGenerateDefaultConfig(t *testing.T) {
	...
	if !strings.Contains(content, "# FIX Core Configuration") {
...
func TestCoerceFloat64ViaJSON(t *testing.T) {
	...
	if cm.GetInt("network.port") != 17464 {
		t.Errorf("expected network.port=17464, got %d", cm.GetInt("network.port"))
	}
}
