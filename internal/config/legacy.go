...
// If a legacy fix.conf path is provided, it returns the default config
...
func LoadConfigAuto(path string) (*Config, error) {
	...
	if strings.HasSuffix(lowerPath, ".conf") {
		logrus.Warnf("legacy fix.conf format is no longer supported, using defaults. Please migrate to fixd.yml (YAML format): %s", path)
