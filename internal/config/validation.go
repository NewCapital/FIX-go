...
func validateLoggingConfig(config *LoggingConfig) error {
	...
	if config.Output == "" {
		return fmt.Errorf("logging output must be 'stdout', 'stderr', or a file name (e.g., 'fix.log')")
	}
...
func validateWalletConfig(config *WalletConfig) error {
	// COIN = 100,000,000 satoshis (1 FIX)
	...
	if config.PayTxFee > COIN {
		return fmt.Errorf("payTxFee cannot exceed 1 FIX (100000000 satoshis) per KB, got %d", config.PayTxFee)
	}

	// Validate MinTxFee (legacy: -mintxfee)
	// Default: 10000 satoshis (0.0001 FIX) per KB
	...
	if config.MinTxFee > COIN {
		return fmt.Errorf("minTxFee cannot exceed 1 FIX (100000000 satoshis) per KB, got %d", config.MinTxFee)
	}

	// Validate MaxTxFee (legacy: -maxtxfee)
	// Default: 1 FIX (100000000 satoshis) - legacy default is 1 COIN
	...
	if config.MaxTxFee > 0 && config.MaxTxFee < 10000 {
		return fmt.Errorf("maxTxFee must be at least 10000 satoshis (0.0001 FIX), got %d", config.MaxTxFee)
	}
	if config.MaxTxFee > 10*COIN {
		return fmt.Errorf("maxTxFee cannot exceed 10 FIX (1000000000 satoshis), got %d", config.MaxTxFee)
	}
...
func validateStakingConfig(config *StakingConfig) error {
	...
	// Range: 0 to 100M FIX (sanity limit to prevent unreasonable values)
	if config.ReserveBalance < 0 {
		return fmt.Errorf("reserveBalance cannot be negative, got %d", config.ReserveBalance)
	}

	// Sanity limit: 100M FIX (100,000,000 * COIN = 10^16 satoshis)
	// This is well below int64 max and covers any reasonable reserve
	maxReserve := int64(100000000) * COIN // 100M FIX
	if config.ReserveBalance > maxReserve {
		return fmt.Errorf("reserveBalance cannot exceed 100M FIX (%d satoshis), got %d", maxReserve, config.ReserveBalance)
	}
