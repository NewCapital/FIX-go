...
| Config format | `fix.conf` (key=value) | `fixd.yml` (YAML) + legacy `fix.conf` support |
...
go build ./cmd/fix-cli    # CLI client
```

### Run
```bash
./fixd start > /dev/null 2>&1 &   # start daemon in background
./fix-cli getinfo                  # check status
```

## Migrating from Legacy Wallet

If you have an existing FIX wallet from the C++ version, copy your `wallet.dat` to the Go daemon's data directory (`~/.fix/`).
